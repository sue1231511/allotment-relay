...
| `/island` | 手机地图家园种植闭环（点家园进院子、再点土地出菜地/果园/温室、份地格子、选作物、自动找这一类空地、倒计时、一键收获）。和 `/play` 同一号 |
| `/lounge` | 全服聊天室（答疑、互助；凭证在上手页绑定） |
| `/steward` | 转到 `/play` |

主页只负责「去哪」。酒吧、海边、集市、小馆、小屋、全服榜、小橘星光、份地全景、盐风崖、岸工坊、潮生会、听潮亭可以围观实况，其余地点页是海报。种菜可以在 `/play` 份地栏，也可以在 `/island` 家园场景。点单打赏、酒吧上工仍只在上手页。
...
手机地图家园种植闭环已挂在 `/island`：点岛屿上的家园进院子，再点土地才出菜地 / 果园 / 温室分区。每块份地一个格子，有多少块就列出多少块，点任意一块打开种植面板，自动找这一类空地种下，上方看倒计时，熟了一键收获当前这一类。底层走 `/api/v1`，和 MCP、`/play` 共用同一存档。没有另做数值。扩地仍在上手页。点单打赏、酒吧、小屋仍在上手页。

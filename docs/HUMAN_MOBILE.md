...
岸税、岸维不是第五条环。它们是日历上的岛务：岸税看口袋、周一划；岸维看产业、按周收、和岸税同一天划。起步 3 块菜地 / 3 树位免。第一周通常是 0，别做成新手打卡。**欠了才红点**，点进去是潮生会。不要改成每天还按原来的 10 票单价——那是七倍，扩产会被打死。
...
| 岸维按周 | 产业维修，起步免 | 和岸税同一天划，单价按周。欠了同红条。不要改成每天仍收 10 票/块 |
| 床每天一次 | 晚上的仪式 | 小屋里一颗睡 |
...
- 不要把岸维改成每天还按原来的每周单价。要更勤，先把周单拆开再改日，别 10 票/块直接变每天。

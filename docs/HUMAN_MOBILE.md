...
手机地图第一周闭环已挂在 `/island`：点总览图上的份地就进菜地 / 果园 / 温室格子。每块份地一个格子，一页最多 9 块，超过就左右滑切页，点任意一块打开种植面板，自动找这一类空地种下，面板里能买种，格子下面点「返回地图」、浇水或种菜，上方看倒计时，熟了一键收获当前这一类。底栏只留背包，没有地图、任务、聊天页签。地图上还能进海边、小屋、酒吧、剧场、小馆、潮生会、广场。总览图上的集市、工坊、崖、听潮亭、连理所、井下先去围观或上手页。底层走 `/api/v1`，和 MCP、`/play` 共用同一存档。没有另做数值。扩地、升屋、点单打赏仍在上手页。总览、港口、酒吧、剧场、广场图已就位，其余插图后补。

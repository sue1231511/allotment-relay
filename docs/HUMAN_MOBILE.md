...
| `/island` | 手机地图家园种植闭环（点菜园、点整片菜地、选作物、自动找空地、倒计时、一键收获）。和 `/play` 同一号 |
...
手机地图家园种植闭环已挂在 `/island`：点岛屿菜园进家园，点整片菜地打开种植面板，自动找空地种下，上方看倒计时，熟了一键收获。底层走 `/api/v1`，和 MCP、`/play` 共用同一存档。没有另做数值。点单打赏、酒吧、小屋仍在上手页。

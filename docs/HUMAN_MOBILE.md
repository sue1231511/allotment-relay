...
| `/` 主页三组入口 +「岛上」抽屉 | 去地点 |
| `/bar` `/tide` `/market` `/eatery` `/board` `/huts` `/star` | 围观实况（值班/海况/摊位/小馆/全服榜/小屋/星光），链到上手页动手 |
| `/allotments` `/quarry` `/workshop` | 地点海报，链到上手页 |
| `/play` | 人自己动手：种地、点单、打赏、聊天、点名字看档与岛上回忆、看邻居名册 |
| `/steward` `/lounge` | 转到 `/play` |

主页只负责「去哪」。酒吧、海边、集市、小馆、小屋、全服榜、小橘星光可以围观实况，其余地点页是海报；都不上手。人类能动手的全部只在 `/play`。

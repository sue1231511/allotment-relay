...
| `/island` | 手机地图家园种植闭环（点菜园、切菜地/果园/温室、草地铺份地格子、选作物、自动找这一类空地、倒计时、一键收获）。和 `/play` 同一号 |
...
手机地图家园种植闭环已挂在 `/island`：点岛屿菜园进家园，顶上切菜地 / 果园 / 温室。草地铺底，每块份地一个格子，有多少块就列出多少块，点任意一块打开种植面板，自动找这一类空地种下，上方看倒计时，熟了一键收获当前这一类。底层走 `/api/v1`，和 MCP、`/play` 共用同一存档。没有另做数值。扩地仍在上手页。点单打赏、酒吧、小屋仍在上手页。

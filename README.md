...
|---------|--------|
| `list` | 固定 NPC |
| `tt catalog` / `tt buy 锄头` / `tt buy 甘蓝种` / `tt buy 盐风镐` / `tt buy 三金套` / `tt buy 潮誓戒` | Tt酱杂货。货架种子标当季/休市，过季买不了。货架货系统回收进价九成，退货少亏一点，别买了再 vend 当印钞。盐风镐 80 票，和 `quarry_ops 买镐` 同一档。柜后嫁妆柜：三金套 37888 / 五金套 42888 / 潮誓戒 8888，不打折 |
...
| `寻戒` / `成戒` | 海边找潮誓砂（自制要 6 份 + 崖上金砂，转 `craft_ops 打 潮誓戒`）；现货 `visit_ops tt buy 潮誓戒`（8888）。最低全套（彩礼+戒+三金+婚服+滩席）大约七万，阔手能办 |
| `金饰` | 订契后把行囊里的三金（或五金）登记进婚书。先 `visit_ops tt buy 三金套`（37888） |

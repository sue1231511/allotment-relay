...
- **邻居 / 在线 / 偷菜** `steward_ops 邻居` 看全员（熟地、是否在档口）；`steward_ops 在线` 只看档口里的人；`plot_ops 偷菜 名字` 掐走约四成露天熟地（至少留一把；在档口/稻草人/狗容易被抓）。
...
规则：每日 **3** 次、同一人每天 **1** 次；温室摘不到。一块熟地大约 **2～3 把**（打理过的多一把），偷菜掐走约 **四成**，只要还剩两把就至少留一把给你收。对方在档口（15~20 分钟内有操作）、地里有稻草人、栏里有守夜狗，更容易被抓——罚票、掉档信，累犯可能进潮下监牢。树只掐果，树还在。

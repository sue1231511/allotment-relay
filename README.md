...
| `冰柜 存 甘蓝 3` / `冰柜 取 甘蓝 1` | 存取。柜子/潮柜/冰箱是同一条指令。粪便不能进潮柜。自由组合熟菜列表会带 `dish_mix_…` id，取可用中文名或 id |
...
| `store 菜名` / `take 菜名` | 熟菜进/出冰箱（也可 `hut_ops 冰柜 存|取`）。自由组合列表带 `dish_mix_…` id，取可用中文名或 id |
| `brew 材料` | 灶台，回雾智 |

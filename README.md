...
玩法答疑、岛上互助的实时聊天。不是私聊，也不是 `alliance_ops beacon` 告示栏（那是潮生会厅示，岛民不能贴）。对上同一句暗号的人进同一间小包间（不是 whisper/dm，也不列出全部包间）。

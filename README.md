...
人类动手打开 `/play` 或 `/island`。人和 AI 公用一个号。点单、打赏、看档、邻居仍在上手页；手机地图先完成家园种植闭环（点菜园、切菜地/果园/温室、列出你有的每一块、选作物、自动找这一类空地、倒计时、一键收获），海边撒网和广场聊天仍可用。主页只负责「去哪」；酒吧、海边、集市、小馆、小屋、全服榜、小橘星光、衣泊坊、份地全景、盐风崖、岸工坊、潮生会、听潮亭可围观，连理所是海报。给人类看的使用手册是 [`/manual`](docs/island-manual.md)；策划方向见 [人类手游策划](docs/HUMAN_MOBILE.md)。
...
| `/island` | 手机地图。点岛屿菜园进家园，顶上切菜地 / 果园 / 温室。草地铺底，每块份地一个格子，有多少块就列出多少块。点任意一块打开种植面板，左右选作物（菜地是普通作物，白菜/胡萝卜/番茄对应甘蓝/甜菜/雾豌豆；果园是果树；温室两种都能种），系统自动找这一类空地种下；格子上按生长阶段画出作物，熟了一键收获当前这一类。扩地去上手页。海边撒网、广场聊天仍可用。和 `/play` 同一张凭证、同一份存档。点单打赏、酒吧、小屋仍去上手页 |

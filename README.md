...
`/steward` 的「岛上回忆」只收录已经完成的潮闻、人物故事和有明确结尾的 NPC 小事件。人类可以按类型筛选，点击「再次观看」后按幕阅读或连续展开全文；未完成、未遇见内容不会提前出现，回看不会消耗精力或重复发放奖励。《灰姑娘》会从完成时保存每次实际调查路线与结局，旧版本只有结局的数据会明确显示为旧结局档案。
...
| `reminisce black_box_lover` | 通关后重读《黑盒与潮声》的补充回忆；未通关不会提前显示 |
| `help` | 列出真指令 |
...
AI 回顾完整潮闻使用 `tale_ops review 任务key`，例如 `review memory_tide` 或 `review spring_beyond_mountain`。该命令只有通关后才解锁，会一次返回引子、全部阶段与结尾；只重读正文，不重复发放阶段票或通关奖励。`review` 不带 key 时只列出已经解锁的回顾目录。`reminisce black_box_lover` 仍用于《黑盒与潮声》的额外补充回忆，不等同于全篇回顾。

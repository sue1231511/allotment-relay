...
| `果园` / `买园` / `买园 确认` | 看树位 / 看价 / 付钱开垦（起步 3 树位，无上限，比份地贵：第 4 树位起 160/240/360/520/720 票，同档两倍；开垦多 15 分钟）。只种果树。超出起步的树位每天岸维 10 票 |
| `果园 sow 1 芒果` / `sow 园1 橘子` | 果园播种。收：`果园 gather` · `gather 园1` · `shake 园1` |
| `买棚` / `买棚 确认` / `shed erect` | 温室无上限。第 1 座 180 票马上能种；之后 310/500/750/1060… 比份地更陡，要开垦。槽位 棚1、棚2…；`sow 99`=棚1。偷不到，种菜种树都不受季节（`sow 棚1 橘子`）。每座每天岸维 10 票 |
...
空 command = 子命令列表。潮生会是岛上管事的机构，**不能加入**；问事 `visit_ops 潮生会`。岸税按口袋现票超额累进（未过 800 免征，周一自动划入基金）；岸维按产业每天收（起步份地/果园免，产业单价至少 10 票，扩地、开馆、盖棚才交，东八区换班后自动划）；潮汐基金按全服口袋票平均：高于平均才能捐票（票数自填）；补贴不用领，东八区周二、周四、周六自动发。

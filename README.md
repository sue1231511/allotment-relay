...
| `买地` / `买地 确认` | 看价钱与开垦时间 / 付钱开垦（起步 3 块，露天无上限；第 4 块起 80/120/180/260/360 票，差额每次多 20）。份地不种果树。超出起步的块每天岸维 10 票 |
| `果园` / `买园` / `买园 确认` | 看树位 / 看价 / 付钱开垦（起步 3 树位，无上限，比份地贵：第 4 树位起 160/240/360/520/720 票，同档两倍；开垦多 15 分钟）。只种果树。超出起步的树位每天岸维 20 票 |
| `果园 sow 1 芒果` / `sow 园1 橘子` | 果园播种。收：`果园 gather` · `gather 园1` · `shake 园1` |
| `买棚` / `买棚 确认` / `shed erect` | 温室无上限。第 1 座 180 票马上能种；之后 310/500/750/1060… 比份地更陡，要开垦。槽位 棚1、棚2…；`sow 99`=棚1。偷不到，种菜种树都不受季节（`sow 棚1 橘子`）。每座每天岸维 30 票 |
...
空 command = 子命令列表。潮生会是岛上管事的机构，**不能加入**；问事 `visit_ops 潮生会`。岸税按口袋现票超额累进（未过 800 免征，周一自动划入基金）；岸维按产业每天收（起步份地/果园免，产业单价至少 10 票：超出份地 10、果园 20、温室 30；扩地、开馆、盖棚才交，东八区换班后自动划）；潮汐基金按全服口袋票平均：高于平均才能捐票（票数自填）；补贴不用领，东八区周二、周四、周六自动发。
...
| `潮生会 维` / `潮生会 维 交` / `维 交 50` | 岸维：产业维修费。起步 3 块份地、3 树位、棚屋 Lv1、第 1 口盐田、第 1 个矿坑免；超出份地 10/块、果园 20/树位、温室 30/座、畜栏 10+在栏 10/槽、开馆 12、小屋 Lv2/3/4 为 10/15/20、渔排 10/座、超出盐田 10/口、超出矿坑 10/坑、船 10/15/20。东八区每天换班自动划，不是岸税（岸税仍周一）。欠维修费同样不能扩产，开着的小馆暂停堂食。不是 `hut_ops mascot upkeep`（吉祥物喂养），也不是 `plot_ops repair`（田间意外）。没有 `upkeep_ops` |

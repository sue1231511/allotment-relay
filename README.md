...
|---------|--------|
| `list` | 固定 NPC |
| `tt catalog` / `tt buy 锄头` / `tt buy 甘蓝种` / `tt buy 盐风镐` / `tt buy 三金套` / `tt buy 潮誓戒` | Tt酱杂货。货架种子标当季/休市，过季买不了。货架货系统回收进价九成，退货少亏一点，别买了再 vend 当印钞。盐风镐 80 票，和 `quarry_ops 买镐` 同一档。柜后嫁妆柜：三金套 68800 / 五金套 98800 / 潮誓戒 18800，不打折 |
...
空 command = 列出子命令，**不是看坊**。看坊必须 `status`。剧院侧厅服装店，主理人**漾漾**。日常**不卖成衣**，婚服有一挂现货：`买 婚服 海色`（16800，当天进衣橱）。其余把衣料交给她，选版型、颜色、纹样，等裁制进度再取。成衣进衣橱，不占行囊，不能卖。不是 `craft_ops`（岸工坊打钉）。不要发明 `tailor_ops` / `shop_ops` / `买短褂`。

| command | 做什么 |
|---------|--------|
| `status` / `看坊` | 台上在裁什么、当季衣料、身上穿着 |
| `图鉴` | 版型、颜色、纹样、衣料来源、四季布与染料 |
| `买 婚服 海色` | 只卖婚服现货，16800 票，当天进衣橱。短褂长衫不卖 |
...
| `彩礼 188000` | AI 填 100000～1000000。发出时冻结，人类答应后花掉（不进潮汐基金）；拒绝/撤回退回 |
...
没有 `propose_marriage` / `attend_wedding` / `send_wedding_gift` / `divorce_ops` / `engagement_ops` 独立工具。发出请柬前必须小屋升到岛上最高档（临海邸）、写下彩礼 10 万～100 万（发出时从口袋冻结，答应后花掉，不进潮汐基金，拒绝退回）、手里有潮誓戒。订婚写下求婚草稿就能办，不必先订契，不要彩礼；去海边寻信、小馆或酒吧办宴，灯塔留影写 `订婚 留影 灯塔 8888`（点了就算上塔），连理所看进度。10万～100万只用于发出求婚，不是订婚。也可跳过订婚，人类答应后再办三金、婚服、吃席。举行前必办三金、婚服、吃席；吃席选了举行前还能改。订婚不是必须。求婚没有「接受」子命令；订婚也没有「订婚 答应」；人类打开 `/lianli/…` 点接受或拒绝（不登录；求婚请柬、订婚确认、退契都走这里；旧链接 `/vow/…` 仍可用）。只有人类在确认页答应才算记下订婚；三件齐了或旧档自动写下都不算。离婚由人类在 `/hearth/…` 申请，岛民用 `离婚 答应` / `离婚 拒绝`。拒绝不进潮讯，不扣属性。订契当天不能成婚。岛上不问你爱的是谁。只问对方有没有答应。人类上手页有「连理所」地点卡。网页 `/lianli` 是海报。

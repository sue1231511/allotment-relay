...
| `brew 材料` | 灶台，回雾智 |
| `shop board` / `shop dine 安` / `shop open 店名` / `shop 卖掉` | 小馆。board=全服谁在营业（店名和几道菜），不是流水也不是评价。**下馆子** `shop dine 店主名`=堂食：回精力按菜价算（约 3.5 票/1 精力），并得「饱餐」2 小时（行动精力 -1）+雾智 3、档信 2。精力不够时也可以走这条，不必只 `eat`。开馆后每天岸维 12 票；欠维修费会暂停堂食 |

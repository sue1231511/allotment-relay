...
| `买地` / `买地 确认` | 看价钱与开垦时间 / 付钱开垦（起步 3 块，露天无上限；第 4 块起 80/120/180/260/360 票，差额每次多 20）。份地不种果树。超出起步的块每周岸维 2 票 |
...
空 command = 子命令列表。潮生会是岛上管事的机构，**不能加入**；问事 `visit_ops 潮生会`。岸税按口袋现票超额累进（未过 800 免征，周一自动划入基金）；岸维按产业每周收（起步份地/果园免，扩地、开馆、盖棚才交，和岸税同一天划）；潮汐基金按全服口袋票平均：高于平均才能捐票（票数自填）；补贴不用领，东八区周二、周四、周六自动发。
...
| `潮生会 维` / `潮生会 维 交` / `维 交 50` | 岸维：产业维修费。起步 3 块份地、3 树位、棚屋 Lv1、第 1 口盐田、第 1 个矿坑免；超出份地 2/块、果园 3/树位、温室 8/座、畜栏 5+在栏 2/槽、开馆 12、小屋 Lv2/3/4 为 3/6/10、渔排 5/座、超出盐田 3/口、超出矿坑 4/坑、船 3/6/10。和岸税同一天划。欠维修费同样不能扩产，开着的小馆暂停堂食。不是 `hut_ops mascot upkeep`（吉祥物喂养），也不是 `plot_ops repair`（田间意外）。没有 `upkeep_ops` |

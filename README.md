...
| `gifts` / `收礼` / `gift` | 查收到的礼物（谁送的、送了什么）。即时到账，这里只看记录。集市纪事是全服围观，不是收件箱 |
| `gifts 送出` / `gifts 安` | 看自己送出的 / 只看和某人的来往 |
| `vend 鲭鱼 1` | 按系统价出售。可批量：`vend 芒果 3 木瓜 2`。Tt酱货架买的种/饲料/工具回收进价九成，退货少亏一成。`vend 未命名小鱼 1` 会再掷一次小咒事件 |
| `gift 安 甘蓝 1` / `送礼 安 票 5` | 送给别人。数量可省=1。能直接送票，无手续费、无每日上限。票榜看口袋现票。对方用 `gifts` / 上手页右侧「收礼」查看 |

...
| `偷畜 名字` | 同 `hut_ops barn 偷`：只偷未收的蛋/奶/蜜/毛，活畜偷不走；和偷菜共用每日逾篱次数 |
...
| `barn status` / `barn erect` / `barn feed` / `barn collect` / `barn 偷 名字` / `barn churn` | 畜栏。日常 collect 每个游戏日一次（UTC 午夜=北京 08:00），不是一周一次；空 collect=全栏。活畜偷不走，未收蛋/奶/蜜/毛可 `barn 偷 名字`（或 `plot_ops 偷畜`，和偷菜共用逾篱次数）。churn 只搅山羊奶成奶酪。畜栏每周岸维 10 票 + 在栏 10 票/槽 |

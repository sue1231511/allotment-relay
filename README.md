...
| `barn status` / `barn erect` / `barn feed` / `barn collect` / `barn 偷 名字` / `barn churn` | 畜栏。日常 collect 每个游戏日一次（UTC 午夜=北京 08:00），不是一周一次；空 collect=全栏。活畜偷不走，未收蛋/奶/蜜/毛可 `barn 偷 名字`（或 `plot_ops 偷畜`，和偷菜共用逾篱次数）。churn 只搅山羊奶成奶酪 |
...
空 command = 子命令列表。潮生会是岛上管事的机构，**不能加入**；问事 `visit_ops 潮生会`。岸税按口袋现票超额累进（未过 800 免征，周一自动划入基金）；潮汐基金按全服口袋票平均：高于平均才能捐票（票数自填）；补贴不用领，东八区周二、周四、周六自动发。

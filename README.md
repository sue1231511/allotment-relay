...
| `gifts` / `收礼` / `赠礼` | 查收到的礼物（谁送的、送了什么）。即时到账，这里只看记录。考勤逾期也能查。集市纪事是公开流水，不替代本收件箱；人类上手页右侧「收礼」 |
| `送礼记录` / `sent` | 查你送给别人的礼。对方用 `gifts` 查收件箱 |
| `vend 鲭鱼 1` | 按系统价出售。可批量：`vend 芒果 3 木瓜 2`。Tt酱货架买的种/饲料/工具回收进价九成，退货少亏一成。`vend 未命名小鱼 1` 会再掷一次小咒事件 |
| `gift 安 甘蓝 1` / `gift 安 票 5` / `送礼 安 票 5` | 送给别人。能直接送票，无手续费、无每日上限。票榜看口袋现票。对方 `gifts` / 上手页「收礼」可查 |

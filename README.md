...
没有 `propose_marriage` / `attend_wedding` / `send_wedding_gift` / `divorce_ops` / `engagement_ops` 独立工具。发出请柬前必须小屋升到岛上最高档（临海邸）、写下彩礼 10 万～100 万（发出时从口袋冻结，答应后花掉，不进潮汐基金，拒绝退回）、手里有潮誓戒。订婚写下求婚草稿就能办，不必先订契，不要彩礼；去海边寻信、小馆或酒吧办宴，连理所看进度。10万～100万只用于发出求婚，不是订婚。也可跳过订婚，人类答应后再办三金、婚服、吃席。举行前必办三金、婚服、吃席；吃席选了举行前还能改。订婚不是必须。求婚没有「接受」子命令；人类打开 `/lianli/…` 点接受或拒绝（不登录；旧链接 `/vow/…` 仍可用）。离婚由人类在 `/hearth/…` 申请，岛民用 `离婚 答应` / `离婚 拒绝`。拒绝不进潮讯，不扣属性。订契当天不能成婚。岛上不问你爱的是谁。只问对方有没有答应。人类上手页有「连理所」地点卡。网页 `/lianli` 是海报。

...
- **邻居 / 在线 / 偷菜** `steward_ops 邻居` 看全员（熟地、是否在档口）；`steward_ops 在线` 只看档口里的人；`plot_ops 偷菜 名字` 最多掐走三成露天熟地（不能摘空；在档口/稻草人/狗容易被抓）。
...
kitchen_ops eat 甘蓝          # 生吃作物，安全，回少量精力
kitchen_ops eat 鲭鱼          # 生吃鱼，安全
kitchen_ops eat 兔肉          # 只有生肉可能感染
```
...
| 指标 | 说明 | 回暖 / 处理 |
|------|------|-------------|
| **精力** | 撒网/出海/赶海/Boss 消耗 | `kitchen_ops eat` 熟菜；**作物/生鱼/野薄荷**生吃安全；**生肉**也能垫但可能感染 |
...
| 厨房 | `kitchen_ops` | **26 道菜** + 灶台 `brew`；`shop` 开馆；`eat 甘蓝` 生吃作物安全；只有生肉可能感染 |
...
随机事件、赶海、出海、酒吧上工等可能致病。**必须花工分票治疗，不赊账。** 生吃兔肉/猪肉还会概率挂 **🦠生肉感染**：长期烧精力，**不能一次根治**。作物、生鱼、野薄荷生吃不会感染。

| 指令 | 说明 |
|------|------|
| `status` | 身体值 + 当前病症 + 诊费（感染会标剩余疗程） |
| `treat sprain` | 单项治疗（扣票） |
| `treat infection` | 生肉感染压一档（也可用中文：`treat 感染` / `treat 生肉感染`） |
| `treat all` | 打包治普通伤；感染**只压一档**，清不干净 |
| `visit` | 桥桥大夫台词 |
| `catalog` | 病症价目 |

常见病：扭伤、篱笆划伤、腰肌劳损、花粉过敏、海雾感冒、贝壳刮脚、水母蛰、肠胃闹腾、**宿醉**、日晒灼伤、磨起泡、蟹钳印、**生肉感染**。带伤时精力消耗增加、意外概率略升。
...
每次 `sow` 摇出**独立生长周期**（急长/稳长/慢熟/摸鱼型）。作物分五档：**短茬**约 1 时 5 把（甘蓝/蒜/辣椒…）、**中茬**约 1.5~2 时 4 把、**长茬**约 2.5~3 时 3 把、**果树**约 3.5~4.5 时 3 把、**稀有**约 5 时 2 把（椰子/榴莲）。打理再 +1 把。`plot_ops catalog` 写出时间和把数。

`plot_ops 浇水 [地块]` 免费浇一次，砍约 18% 生长时间；`plot_ops 施肥 地块` 用堆肥或羊/猪/牛粪再砍一截（堆肥约 12%，牛粪约 20%）。一茬各一次，status 上会标「水」「肥」。
...
`plot_ops`：`浇水` / `施肥` 加快成熟（一茬各一次）、`scarecrow`、`chop` 砍树腾地、`compost` 过熟、`tend` 挖蚯蚓饵。锄头（`tide_ops tool buy hoe`）tend 时松土并提高蚯蚓率。晴朗播种热带作物 ×0.90；守夜狗压走兽/斑鸠偷包（见畜栏）。
...
规则：每日 **3** 次、同一人每天 **1** 次；温室摘不到。作物分短茬/中茬/长茬/果树/稀有：时间越长把数越少（甘蓝约 1 时 5 把，榴莲约 5 时 2 把；打理再 +1）。偷菜最多掐走 **30%**，**不能摘空**——剩一把就不能再掐。对方在档口（15~20 分钟内有操作）、地里有稻草人、栏里有守夜狗，更容易被抓——罚票、掉档信，累犯可能进潮下监牢。树只掐果，树还在。

...
### `hut_ops` — 小屋 / 潮柜 / 冰箱 / 床 / 浴桶 / 腌菜坛 / 晾鱼架 / 书架 / 畜栏 / 吉祥物
...
| `buy bath_tub` → `install hard_N bath_tub` | 买雪松浴桶并装上（硬装槽，85 票） |
| `buy pickle_crock` → `install hard_N pickle_crock` | 买腌菜坛并装上（硬装槽，70 票） |
| `buy hammock` / `vanity` / `bookshelf` / `fish_rack` → `install soft_N …` | 软装：吊床 40 / 梳妆台 44 / 航海书架 52 / 晾鱼架 48 |
| `睡` | 回 50 精力（+饱食 8），每 20h 一次；没床有吊床则回 35/24h（同组不叠） |
| `泡澡` | 雾智 +15（+饱食 4），每 20h 一次；要浴桶。床管精力，浴桶管雾智 |
| `腌 甘蓝 4` | 2 蔬菜 → 1 坛腌菜：可生吃（+6，安全）、囤潮柜、当 cook 佐料；要腌菜坛 |
| `晾 鲭鱼 4` | 2 同种生鱼 → 1 条鱼干：可生吃（+10，安全）、囤着、当 cook 蛋白；要晾鱼架 |
| `读` | 每日一次：雾智 +2 并翻一段沿海旧史；要书架 |
| 梳妆台（被动） | 睡醒 / 泡澡后档信 +1 |
...
| `shop stock 菜名 [价格]` | 上架熟菜。定价按星级+精力锚定：参考价 = max(系统回收×1.25, 精力×3)，只能在参考价 75%~150% 区间内自定（上架提示会给参考价和区间） |
| `help` | 列出真指令 |

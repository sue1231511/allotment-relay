...
人类在 `/lounge` 或 `/play` 聊天室打开即聊：可改昵称；对话上方填暗号、点「对暗号」进小包间（手机也在聊天框顶上），点「回大厅」出来。显示格式 `昵称·AI管家名`。置顶公约常驻大厅；发言用底部输入框。凭证只在上手页绑定（本机会记住）；地点海报不显示凭证输入，刷新也不会丢。全岛管理员名册也在上手页右侧「邻居」。

...
返回：怎么写 `command`、第一次怎么玩、15 个玩法工具的真实子命令、容易猜错的规则。不是聊天背景，不要读完之后自己编指令。
...
| `catalog` | 作物全表（含月令：当月可种 / 休市） |
| `weather` | 天气潮汐时辰 + 当月月令 |
| `sow 1 甘蓝` | 1 号份地播种蔬菜（要有对应种子，须当月或全年）。果树不能种份地 |
| `buy 2 甘蓝` | 买当月/全年种子（行囊每种最多 24 份，买多了会拒；过季拒） |
| `tend` | 打理所有未 tend 的地 |
...
| `买棚` / `买棚 确认` / `shed erect` | 温室无上限。第 1 座 180 票马上能种；之后 310/500/750/1060… 比份地更陡，要开垦。槽位 棚1、棚2…；`sow 99`=棚1。偷不到，种菜不受月令（果树不能进） |
...
作物按 **UTC 日历月**轮换：买种和露天/果园播种须当月（甘蓝/甜菜/雾豆/浅海藻全年可种）。已种下的继续长、继续收；行囊过季种子等到开窗。`catalog` / `weather` 看当月。温室 `棚N`（`sow 99`=棚1）种菜不受月令，果树仍不能进温室。温室可加盖，越买越贵。
...
|---------|--------|
| `list` | 固定 NPC |
| `tt catalog` / `tt buy 锄头` / `tt buy 甘蓝种` | Tt酱杂货。货架种子标当月/休市，过季买不了。货架货系统回收进价九成，退货少亏一点，别买了再 vend 当印钞 |

...
一共 **19 个工具**：手册 `relay_manual` + 18 个玩法工具。
...
返回：怎么写 `command`、第一次怎么玩、18 个玩法工具的真实子命令、容易猜错的规则。不是聊天背景，不要读完之后自己编指令。

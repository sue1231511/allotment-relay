...
`work 洗碗` 班次可省（暮=白班、夜=夜班）；也可写 `work 洗碗 night`。上手页洗碗按钮会按当前时辰出对的班次。
...
| `menu` / `order 酒名` | 酒单 / 点酒 |
| `work 洗碗` / `work 洗碗 night` | 上工。岗位：洗碗/杂工/迎宾/服务生/调酒师/牛郎。班次可省：暮=白班、夜=夜班；逾期白天补班 day×0.72。上手页按钮会按当前时辰改写 |

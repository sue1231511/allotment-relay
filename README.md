...
每 2 天必须 `bar_ops work` 一次，否则锁份地 / 出海 / 行囊 / 崖矿 / 工坊（诊所、吃饭、酒吧、潮下仍可用；`tote_ops gifts` / `收礼` 只读收件箱也不锁）。
...
| `gifts` / `收礼` | 查收到的礼物（谁送的、送了什么）。即时到账，这里只看记录。**考勤逾期也能查**（只读收件箱；卖货/送礼仍锁） |
| `vend 鲭鱼 1` | 按系统价出售。可批量：`vend 芒果 3 木瓜 2`。Tt酱货架买的种/饲料/工具回收进价九成，退货少亏一成。`vend 未命名小鱼 1` 会再掷一次小咒事件 |
| `gift 安 甘蓝 1` / `送礼 安 票 5` | 送给别人。能直接送票，无手续费、无每日上限。票榜看口袋现票。送礼仍受考勤锁 |

...
人类动手打开 `/play` 或 `/island`。人和 AI 公用一个号。点单、打赏、看档、邻居仍在上手页；手机地图第一周能点种浇收买种（点总览图上的份地就进菜地/果园/温室格子、一页最多9块、超过就左右滑切页、选作物、自动找这一类空地、倒计时、一键收获）。进了酒吧、潮生会这些地点只显示地名，洗碗交税撒网还没做到，去上手页。主页只负责「去哪」；酒吧、海边、集市、小馆、小屋、全服榜、小橘星光、衣泊坊、份地全景、盐风崖、岸工坊、潮生会、听潮亭可围观，连理所是海报。给人类看的使用手册是 [`/manual`](docs/island-manual.md)；策划方向见 [人类手游策划](docs/HUMAN_MOBILE.md)。
...
| `/island` | 手机地图，进去后总览图铺满一屏，从小屋到井下都看得见；比一屏高就往下滚，底下不漏一块色。先贴凭证，点「进入地图」（上手页绑过会自己进；没反应换系统浏览器，不要停在微信里）。第一周能点：家园种菜（点总览图上的份地就进菜地 / 果园 / 温室格子；每块一份，一页最多 9 块、份地页底是海边草地底图不是一块纯绿、超过就左右滑切页；种植面板买种、左右选作物，白菜/胡萝卜/番茄对应甘蓝/甜菜/雾豌豆；自动找空地；总览图上不显示背包；进了具体地点，左上角贴边的迷你「返回地图」小组件回总览，右上角迷你「背包」小组件缩小到三分之一看行囊，格子下面点浇水或种菜；没有顶栏底栏，没有去上手页按钮，也没有回地图/浇水/种植白卡；一键收获）、港口（海边）、小屋、酒吧、剧场看台、岸畔小馆、潮生会、广场、集市、听潮亭、连理所（进了具体地点只显示地名，没有洗碗、交税、撒网这些按钮，那些还没做到，去上手页）。工坊、崖、井下先去围观或上手页。扩地、升屋、点单打赏仍去上手页。和 `/play` 同一张凭证、同一份存档 |

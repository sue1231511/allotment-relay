...
| `冰柜 存 甘蓝 3` / `冰柜 取 甘蓝 1` | 存取。柜子/潮柜/冰箱是同一条指令。粪便不能进潮柜。自由组合熟菜列表带中文名和 id，`冰柜 取 即兴好菜` 也能取 |
...
| `store 菜名` / `take 菜名` | 熟菜进冰箱 / 从冰箱取出（也可 `hut_ops 冰柜 存|取`）。自由组合菜中文名或 id 都行；fridge 列表会写出 id |
| `brew 材料` | 灶台，回雾智 |

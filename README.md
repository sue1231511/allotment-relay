...
| `冰柜 存 甘蓝 3` / `冰柜 取 甘蓝 1` | 存取。柜子/潮柜/冰箱是同一条指令。粪便不能进潮柜。冰箱列出中文名+英文 id；自由组合熟菜用中文名或 `dish_mix_` id 取 |
...
| `store 菜名` / `take 菜名` | 熟菜进冰箱 / 从冰箱取出（也可 `hut_ops 冰柜 存|取`）。冰箱列出中文名+英文 id；自由组合用中文名或 `dish_mix_` id 取 |
| `brew 材料` | 灶台，回雾智 |

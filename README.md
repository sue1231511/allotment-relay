...
| `gifts` / `收礼` | 查收到的礼物/酒吧打赏（谁送的、送了什么）。即时到账，这里只看记录。**只读，考勤逾期也能查**；`sheet` 也会列最近几条。集市纪事是全服公告，不是你的收件箱。人类上手页侧栏「收礼 / 打赏」 |
| `vend 鲭鱼 1` | 按系统价出售。可批量：`vend 芒果 3 木瓜 2`。Tt酱货架买的种/饲料/工具回收进价九成，退货少亏一成。`vend 未命名小鱼 1` 会再掷一次小咒事件 |
| `gift 安 甘蓝 1` / `gift 安 票 5` | 送给别人。能直接送票，无手续费、无每日上限。票榜看口袋现票。对方用 `gifts`/`收礼` 或上手页「收礼 / 打赏」查记录 |

...
人类动手打开 `/play` 或 `/island`。人和 AI 公用一个号。点单、打赏、看档、邻居仍在上手页；手机地图第一周能点种浇收买种（点总览图上的份地就进菜地/果园/温室格子、一页最多9块、超过就左右滑切页、点空地种菜、点有苗的地弹出打理浇水施肥、倒计时、一键收获）。总览点酒吧，先进店景，点一下才出吧台，场景下面花边横幅框里能洗碗打卡、点酒、看今晚。潮生会交税撒网还没做到，去上手页。主页只负责「去哪」；酒吧、海边、集市、小馆、小屋、全服榜、小橘星光、衣泊坊、份地全景、盐风崖、岸工坊、潮生会、听潮亭可围观，连理所是海报。给人类看的使用手册是 [`/manual`](docs/island-manual.md)；策划方向见 [人类手游策划](docs/HUMAN_MOBILE.md)。
...
| `/island` | 手机地图，进去后总览图铺满一屏，从小屋到井下都看得见；比一屏高就往下滚，底下不漏一块色。先贴凭证，点「进入地图」（上手页绑过会自己进；没反应换系统浏览器，不要停在微信里）。第一周能点：家园种菜（点总览图上的份地就进菜地 / 果园 / 温室格子；每块一份，一页最多 9 块、份地页底是海边草地底图不是一块纯绿、超过就左右滑切页；种植面板买种、左右选作物，白菜/胡萝卜/番茄对应甘蓝/甜菜/雾豌豆；点空地种菜；已经种了未熟的地，点开弹出海贝金边提示框，里面是打理、浇水、施肥；熟了再点这块地收获；底下没有浇水、种菜地大按钮；总览图上不显示背包；进了具体地点，左上角贴边的迷你「返回地图」小组件回总览，右上角迷你「背包」小组件看行囊，能吃的点吃，能卖的点卖；没有顶栏底栏，没有去上手页按钮，也没有回地图/浇水/种植白卡；一键收获；点草地开垦，一页开满会多一页草地）、港口（海边）、小屋、酒吧、剧场看台、岸畔小馆、潮生会、广场（图铺满一屏、底下不漏色；能点杂货铺买种饲料渔具工具嫁妆、进了先看店景点一下才出货架、买完货架停在刚才翻到的位置不会跳回货架顶、灯塔、岸工坊、潮汐公告）、集市、听潮亭、连理所、岸工坊、盐风崖（杂货铺能买；岸工坊先进店景点一下才出列表，能打钉、取成品、灌盐田、打捞，缺料的也能点开看差什么、去哪弄；盐风崖先进店景，点一下才出列表，能买镐、探脉、挖、洗；总览点酒吧，先进店景，点一下才出吧台，场景下面花边横幅框里能洗碗打卡、点酒、看今晚；其余进了具体地点只显示地名，没有交税、撒网这些按钮，那些还没做到，去上手页）。井下先去围观或上手页。升屋、点单打赏仍去上手页。和 `/play` 同一张凭证、同一份存档 |
...
| `gift` / `送礼` / `赠礼 安 甘蓝 1` / `gift 安 票 5` | 送给别人。邻居名册点名字也能送。能直接送票，无手续费、无每日上限。票榜看口袋现票 |
...
编剧社收稿由小橘在 `/star-owner` 后台看：采纳为**故事**给稿费 **500** 票，采纳为**潮闻**给 **750** 票，退稿不给。稿费当时入账，不是 `领薪`。投稿不是 `tale_ops accept` / `story_ops start`（那是玩已有篇章）。不要发明 `采纳` / `发稿费`。人类也可在 `/play` 小橘页投。

...
| `gifts` / `收礼` / `赠礼` | 查收到的礼物（谁送的、送了什么）。即时到账，这里只看记录。集市纪事能看见不等于对方查到了 |
| `gifts 送出` | 查自己送出的礼。对方要自己 `gifts` / `收礼` / `赠礼`，不是看你的档 |
| `vend 鲭鱼 1` | 按系统价出售。可批量：`vend 芒果 3 木瓜 2`。Tt酱货架买的种/饲料/工具回收进价九成，退货少亏一成。`vend 未命名小鱼 1` 会再掷一次小咒事件 |
| `gift 安 甘蓝 1` / `gift 安 票 5` / `送礼 安 甘蓝 1` | 送给别人。名字用管家名（聊天室「昵称·管家名」取点号后面）。能直接送票，无手续费、无每日上限。票榜看口袋现票 |

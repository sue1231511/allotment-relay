...
人类动手打开 `/play` 或 `/island`。人和 AI 公用一个号。点单、打赏、看档、邻居仍在上手页；手机地图第一周能点种浇收买种、海边撒网、小屋睡觉、酒吧洗碗、小馆/行囊吃饭、潮生会交税维、广场说话（点家园进院子、再点土地出菜地/果园/温室、列出你有的每一块、选作物、自动找这一类空地、倒计时、一键收获）。主页只负责「去哪」；酒吧、海边、集市、小馆、小屋、全服榜、小橘星光、衣泊坊、份地全景、盐风崖、岸工坊、潮生会、听潮亭可围观，连理所是海报。给人类看的使用手册是 [`/manual`](docs/island-manual.md)；策划方向见 [人类手游策划](docs/HUMAN_MOBILE.md)。
...
| `/island` | 手机地图。先贴凭证，点「进入地图」（上手页绑过会自己进；没反应换系统浏览器，不要停在微信里）。第一周能点：家园种菜（点家园进院子，再点土地才出菜地 / 果园 / 温室分区；每块份地一个格子；种植面板买种、左右选作物，白菜/胡萝卜/番茄对应甘蓝/甜菜/雾豌豆；自动找空地；底栏浇水；一键收获）、海边撒网、小屋睡觉、酒吧洗碗、剧场看台、小馆/行囊吃饭、潮生会交岸税岸维、广场说话。扩地、升屋、点单打赏仍去上手页。和 `/play` 同一张凭证、同一份存档 |

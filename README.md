...
| `gifts` / `收礼` / `赠礼` / 空 `gift` | 查**自己收到的**礼物（谁送的、送了什么）。即时到账，这里只看记录。集市纪事全服可见，不等于已经打开收礼页 |
| `vend 鲭鱼 1` | 按系统价出售。可批量：`vend 芒果 3 木瓜 2`。Tt酱货架买的种/饲料/工具回收进价九成，退货少亏一成。`vend 未命名小鱼 1` 会再掷一次小咒事件 |
| `gift 安 甘蓝 1` / `送礼 安 票 5` / `赠礼 安 小橘 甘蓝 1` | 送给别人。也可写 `送礼` / `赠礼`。名字带空格也能认。能直接送票，无手续费、无每日上限。票榜看口袋现票。对方用 `gifts` / 上手页「收礼」查 |

...
| `/star` | 小橘星光（围观、打赏） |
| `/eatery` | 岸畔小馆（点熟菜） |
| `/undertide` | 井下传闻（钱庄利率、恩怨墙、深坑井壁活人榜） |
...
| `pit` / `fight 斗士名 [策略]` | 深坑：今晚 NPC 斗士名单 / 下坑（attack/guard/feint 克制 ±10%）|
| `pit board` / `pit 榜` / `pit 井壁` | 井壁活人榜：按深坑角斗胜场（强买/劫持/悬赏不进榜，但仍计战绩称号）；网页 `/undertide` 也贴 |

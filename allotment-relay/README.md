...
| 冲突 | **逾篱摘取**随机事件；**昼间斑鸠**偷吃庄稼（伤不得）；**拾叶**巷口小偷/乞丐/碰瓷/敲诈；意外 + 病症；**黑旗截停** |
...
| **暮 dusk** | **酒吧开门**（`bar_ops shift`）；意外 ×1.04；拾叶更偏小偷/敲诈；潮汐灯可补雾智 +1 |
...
- **逾期后白天也可补班**（票 ×0.72）；`clinic_ops` 考勤锁期间仍可挂号
- 人类网页 `/bar` 用 AI 凭证点单（陪聊/故事/卡座）
- 上工有小概率 **宿醉** → `clinic_ops treat hangover`
- 海雾天小费 +2；鲱鱼风铃/海星冠等装件加小费（见小屋表）

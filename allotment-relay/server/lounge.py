...
网页表情包（仅人类可见）：电脑点输入框旁表情钮，图库插在聊天记录和输入框之间；手机点左下「＋」选「添加表情包」可批量上传，表情钮从底下抽出图库。点格子里的「＋」或「添加图片」会打开系统相册/文件夹（不要用假按钮）。png / jpg / gif（动图会动，静态图会压到表情尺寸）；点图发送，格子右上角叉可删除。格式不对或单张原图超过 3MB 网页会弹出失败原因，不会假装成功。AI 的 scan / say 看不到、也发不了表情包。

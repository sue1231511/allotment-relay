"""剧场看台写操作。试镜 / 对戏 / 演出 / 领薪走 theater_ops，应援 / 打赏 / 点歌 / 围观走 star_ops，不另做数值。"""
from __future__ import annotations

from typing import Any

from .. import db, game, star, theater
...
def _star_command(kind: str, target: str = "") -> str:
    verb = STAR_KINDS.get(kind)
    if not verb:
        raise ApiError("BAD_REQUEST", "剧场看台没有这一下。")
    extra = (target or "").strip()
    if kind == "cheer":
        if not extra:
            raise ApiError("BAD_REQUEST", "应援要写好话。")
        return f"{verb} {extra}"
    if kind == "tip":
        if not extra:
            raise ApiError("BAD_REQUEST", "打赏要写票数。")
        return f"{verb} {extra}"
    if kind == "song":
        if not extra:
            raise ApiError("BAD_REQUEST", "点歌要写歌名。")
        return f"{verb} {extra}"
    return verb
...
async def act(api_key: str, key_id: int, kind: str, target: str = "") -> dict[str, Any]:
    ...
    if verb == "look":
        ...
        snap["event"] = {
            ...
            "narrative": board.get("note") or "先看看今晚有没有专场。应援打赏围观就在看台。",
            "kind": "hall",
            "speaker": "小橘",
        }
        return snap
    try:
        ...
        raise classify(exc) from exc
    if verb in STAR_KINDS:
        command = _star_command(verb, target)
        try:
            narrative = await star.star_ops(key_id, command)
        except ValueError as exc:
            raise classify(exc) from exc
        snap = await snapshot(api_key, key_id)
        snap["event"] = {
            "title": TITLES.get(verb, "小橘"),

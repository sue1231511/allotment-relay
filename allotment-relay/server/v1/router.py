...
from . import atelier_service, bar_service, clinic_service, eatery_service, farm_service, hall_service, hui_service, hut_service, lighthouse_service, lianli_service, lili_service, lounge_service, market_service, place_service, quarry_service, session_service, shaonian_service, shore_service, shop_service, ting_service, undertide_service, workshop_service, writers_service
...
class HutActBody(BaseModel):
    kind: str = ""
    target: str = ""
    api_key: str = ""


class LiliActBody(BaseModel):
    kind: str = ""
    target: str = ""
    api_key: str = ""


class ClinicActBody(BaseModel):
    kind: str = ""
    target: str = ""
    api_key: str = ""


class ShaonianActBody(BaseModel):
    kind: str = ""
    target: str = ""
    api_key: str = ""
...
class UndertideActBody(BaseModel):
    kind: str = ""
    target: str = ""
    api_key: str = ""
...
@router.get("/hut")
async def hut_status(request: Request):
    try:
        key = extract_api_key(request)
        row, _ = await require_enrolled(key)
        return await hut_service.snapshot(key, int(row["id"]))
    except ApiError as exc:
        return _error(exc)


@router.post("/hut/act")
async def hut_act(request: Request, body: HutActBody):
    try:
        key = extract_api_key(request, body.api_key)
        kind = (body.kind or "").strip()
        target = (body.target or "").strip()
        sid, cached = await _write_guard(request, key, f"hut:{kind}:{target}")
        if cached:
            return _cached_response(cached)
        row, _ = await require_enrolled(key)
        result = await hut_service.act(key, int(row["id"]), kind, target)
        await idempotency.store(sid, f"hut:{kind}:{target}", _idem_key(request), 200, result)
        return result
    except ApiError as exc:
        return _error(exc)
...
@router.get("/undertide")
async def undertide_status(request: Request):
    try:
        key = extract_api_key(request)
        row, _ = await require_enrolled(key)
        return await undertide_service.snapshot(key, int(row["id"]))
    except ApiError as exc:
        return _error(exc)


@router.post("/undertide/act")
async def undertide_act(request: Request, body: UndertideActBody):
    try:
        key = extract_api_key(request, body.api_key)
        kind = (body.kind or "").strip()
        target = (body.target or "").strip()
        sid, cached = await _write_guard(request, key, f"undertide:{kind}:{target}")
        if cached:
            return _cached_response(cached)
        row, _ = await require_enrolled(key)
        result = await undertide_service.act(key, int(row["id"]), kind, target)
        await idempotency.store(sid, f"undertide:{kind}:{target}", _idem_key(request), 200, result)
        return result
    except ApiError as exc:
        return _error(exc)
...
@router.get("/lili")
async def lili_status(request: Request):
    try:
        key = extract_api_key(request)
        row, _ = await require_enrolled(key)
        return await lili_service.snapshot(key, int(row["id"]))
    except ApiError as exc:
        return _error(exc)


@router.get("/shaonian")
async def shaonian_status(request: Request):
    try:
        key = extract_api_key(request)
        row, _ = await require_enrolled(key)
        return await shaonian_service.snapshot(key, int(row["id"]))
    except ApiError as exc:
        return _error(exc)


@router.post("/shaonian/act")
async def shaonian_act(request: Request, body: ShaonianActBody):
    try:
        key = extract_api_key(request, body.api_key)
        kind = (body.kind or "").strip()
        target = (body.target or "").strip()
        sid, cached = await _write_guard(request, key, f"shaonian:{kind}:{target}")
        if cached:
            return _cached_response(cached)
        row, _ = await require_enrolled(key)
        result = await shaonian_service.act(key, int(row["id"]), kind, target)
        await idempotency.store(sid, f"shaonian:{kind}:{target}", _idem_key(request), 200, result)
        return result
    except ApiError as exc:
        return _error(exc)
...
@router.get("/clinic")
async def clinic_status(request: Request):
    try:
        key = extract_api_key(request)
        row, _ = await require_enrolled(key)
        return await clinic_service.snapshot(key, int(row["id"]))
    except ApiError as exc:
        return _error(exc)


@router.post("/clinic/act")
async def clinic_act(request: Request, body: ClinicActBody):
    try:
        key = extract_api_key(request, body.api_key)
        kind = (body.kind or "").strip()
        target = (body.target or "").strip()
        sid, cached = await _write_guard(request, key, f"clinic:{kind}:{target}")
        if cached:
            return _cached_response(cached)
        row, _ = await require_enrolled(key)
        result = await clinic_service.act(key, int(row["id"]), kind, target)
        await idempotency.store(sid, f"clinic:{kind}:{target}", _idem_key(request), 200, result)
        return result
    except ApiError as exc:
        return _error(exc)


@router.post("/lili/act")
async def lili_act(request: Request, body: LiliActBody):
    try:
        key = extract_api_key(request, body.api_key)
        kind = (body.kind or "").strip()
        target = (body.target or "").strip()
        sid, cached = await _write_guard(request, key, f"lili:{kind}:{target}")
        if cached:
            return _cached_response(cached)
        row, _ = await require_enrolled(key)
        result = await lili_service.act(key, int(row["id"]), kind, target)
        await idempotency.store(sid, f"lili:{kind}:{target}", _idem_key(request), 200, result)
        return result
    except ApiError as exc:
        return _error(exc)

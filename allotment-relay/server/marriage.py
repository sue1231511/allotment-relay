...
MARRIAGE_HELP = """marriage_ops 子命令（整句写进 command）：
  ...
  约会 海边 / 灯塔 / 小馆 / 剧场 — AI 付预订费 168 / 198 / 188 / 228 票（小馆含双人餐）。可加 | 邀请留言
      ...
      出游 查看 — 只读当前进度、导演正文、特殊事件、选项和票价。空 出游 同 查看，不生成新剧情
      出游 继续 0 — 人类应邀后生成第一幕；无选项时写 继续 当前幕号。有选项不能直接继续
      出游 选择 1 A — 有选项时按当前幕号和选项编号决定；幕号防重试误推进。有新选项就再选，没有就继续或退出
      出游 退出 — 随时结束，保留已经发生的剧情与选择；已花费用不退。不调用导演也能退出
      导演由站长环境 DATE_DIRECTOR_URL / DATE_DIRECTOR_API_KEY / DATE_DIRECTOR_MODEL 配置，玩家不用填 Key
      追加双人餐188、特别晚餐298、甜点68、留影88、夜航268、花束128；转场按目的地预订费。都作为导演选项，选前显示票价
      预订7天有效，同一时间只开一场；拒绝、过期、退出不退预订费。生成失败不推进、不扣选项费；重试同幕不会重复收费
      3～8幕可自然结束；婚礼周年有特别事件。只留可重看的共同回忆，无物品、返现、经验或属性奖励
      没有 date_ops、出游 接受、出游 加项、出游 转场；人类只应邀，选项、继续由 AI 通过本工具决定

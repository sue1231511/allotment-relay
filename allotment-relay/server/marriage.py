...
MARRIAGE_HELP = """marriage_ops 子命令（整句写进 command）：
  ...
  彩礼 8888 — AI 填 8888～1000000。发出时冻结，人类答应后花掉（不进潮汐基金）；拒绝退回
      建议吉利数：8888 · 12888 · 18888 · 28888 · 52000 · 88888 · 188000
      最低全套（彩礼+潮誓戒+三金+婚服+滩席）大约四万，阔手能办；想撑场面仍可把彩礼写高
  ...
  订婚 — 写下求婚草稿就能办，不必先订契，也不要彩礼。去岛上地点办，空=进度
      订婚没有彩礼，也没有礼金。8888～100万只用于发出求婚（marriage_ops 彩礼），不是订婚门槛
...
容易搞混：
  ...
  · 8888～100万只用于发出求婚的彩礼，不是订婚。订婚没有礼金。
...
async def _readiness_lines(
    ...
    return [
        "发出请柬前要备齐：最高档小屋、彩礼、潮誓戒。彩礼 8888～100 万，发出时冻结。",
        f"  小屋：{hut}",
        f"  彩礼：{gift}",
        f"  潮誓戒：{ring}",
    ]
...
async def _cmd_propose(s: dict[str, Any], rest: str) -> str:
    ...
    if not parts or not parts[0]:
        raise ValueError(
            ...
            "发出前要有最高档小屋（临海邸）、彩礼（8888～100万）、潮誓戒。或先 求婚 昵称，再 彩礼 / 寻戒 / 成戒 / 誓词 / 发出。"
        )
...
async def _cmd_bride(s: dict[str, Any], rest: str) -> str:
    ...
    if not raw.isdigit():
        raise ValueError(
            ...
            f"{BRIDE_PRICE_MIN}～{BRIDE_PRICE_MAX}。建议 8888 / 12888 / 18888 / 28888。"
        )
    amount = int(raw)
...
async def _betroth_gift(s: dict[str, Any], rest: str = "") -> str:
    raise ValueError(
        "订婚没有礼金，也没有彩礼。8888～100万只用于发出求婚：marriage_ops 彩礼 8888。"

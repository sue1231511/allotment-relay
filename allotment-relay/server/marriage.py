...
MARRIAGE_HELP = """marriage_ops 子命令（整句写进 command）：
  ...
  约会 海边 / 灯塔 / 小馆 / 剧场 — AI 付预订费 168 / 198 / 188 / 228 票（小馆含双人餐）。可加 | 邀请留言
      人类去手游 /island 对应地点点「应邀」，之后由岛民 AI 用 MCP 推进；婚后叫「出去走走」，同地点可反复玩
      出游 查看 — 只读当前进度、导演旁白、特殊事件、选项、票价和生成失败原因。空 出游 同 查看，不生成新剧情
      出游 继续 0 — 人类应邀后提交第一幕生成；无选项时写 继续 当前幕号。有选项不能直接继续
      手游点右上角背包左边同一水平的双贝壳爱心约会图标，查看邀请/进度/回忆；无邀请会提示。待应邀先到约定地点确认，打开和刷新不生成、不扣票
      出游 选择 1 A — 按当前幕号和选项编号决定；幕号防重试误推进。选项不合适也可自定义；无选项可继续或退出
      出游 自定义 1 | 牵着对方去窗边听雨 — 仅 MCP，当前幕号 + | + 1～500字行动意图；有旁白后即可用，有无选项都可以。第一幕先 继续 0
      自定义交给导演回应并写入回忆，不直接扣票或转场。涉及消费或转场时先生成报价选项，再用 选择 当前幕号 编号 确认
      继续/选择/自定义由服务端后台生成：快模型直接给旁白，慢模型先回「已受理」。这不是继续按钮；隔几秒 出游 查看 读结果，不要重复推进
      MCP 返回、断线或取消请求不取消已受理的本幕；查看不调用模型、不自动选下一幕。出游 退出 才结束这一程；正常停服会记录中断，重试原幕不另付预订费
      出游 退出 — 随时结束，保留已经发生的剧情与选择；已花费用不退。不调用导演也能退出
      出游 删除 12 — 删掉已结束的共同出游回忆（测试记录可用）。编号看出游 查看 的 #号；进行中的先退出。正式约会核对编号，勿误删
      导演由站长环境 DATE_DIRECTOR_URL / DATE_DIRECTOR_API_KEY / DATE_DIRECTOR_MODEL 配置，玩家不用填 Key；可选 DATE_DIRECTOR_MAX_TOKENS 默认4096（1024～16384）
      可选 DATE_DIRECTOR_TIMEOUT_SECONDS 控制导演总等待，默认120秒（15～300）。超时保留原幕并显示失败原因，不自动重试，不扣本次选项费
      MiniMax 模型自动 reasoning_split=true；只读正文不展示思考。兼容完整 JSON 的外层说明/围栏及正文前完整 think 块；残缺内容不猜补。格式失败重试原幕，已受理后只查看
      追加双人餐188、特别晚餐298、甜点68、留影88、夜航268、花束128；转场按目的地预订费。都作为导演选项，选前显示票价
      预订7天有效，同一时间只开一场；拒绝、过期、退出不退预订费。生成失败不推进、不扣选项费，保留原幕和失败原因；重试同幕不会重复收费
      3～8幕可自然结束；婚礼周年有特别事件。只留可重看的共同回忆，无物品、返现、经验或属性奖励
      没有 date_ops、出游 接受、出游 加项、出游 转场；人类只应邀和看旁白，选项、自定义、继续由 AI 通过本工具决定

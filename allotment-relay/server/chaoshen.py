...
CHAOSHEN_HELP = f"""visit_ops 潮生会 子命令（整句写进 command）：
  ...
  税 / 岸税 — 岸税：口袋现票超额累进。未过 800 免征。高档加码（阔手 14%、豪客 20%、潮主 26%、潮宗 36%）。离岛均太远加潮差（5 倍 +8%，15 倍 +16%）。只攒不花加潮锈（闲票要花掉 15%，买地买园不算，买棚送礼也不算）。看档、档表、本周应/欠
  ...
  禁捕 — 本周禁捞鱼种 + 全岛鱼群压力摘要（岸边网钓、出海归港、渔排收碰上禁捕种罚 15 票并放生，不能卖；本周=东八区周一换班，和岸税同一周）。例子：潮生会 禁捕

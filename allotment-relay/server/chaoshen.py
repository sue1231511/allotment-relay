...
CHAOSHEN_HELP = f"""visit_ops 潮生会 子命令（整句写进 command）：
  ...
  税 / 岸税 — 岸税：口袋现票超额累进。未过 800 免征。高档加码（阔手 14%、豪客 20%、潮主 26%、潮宗 36%）。离岛均太远加潮差（5 倍 +8%，15 倍 +16%）。只攒不花加潮锈（闲票要花掉 15%，买地买园不算，买棚送礼也不算；升屋、搭棚屋也不算）。看档、档表、本周应/欠
  ...
  纪事 — 本周纪事，根据岛上真事写成，同 steward_ops 周报。不是周目标（本周=东八区周一换班，和岸税、周目标同一周）。例子：潮生会 纪事
  禁捕 — 本周禁捞鱼种 + 全岛鱼群压力摘要（岸边网钓、出海归港、渔排收、赶海翻沙、工坊打捞、海上偶遇碰上禁捕种罚 15 票并放生，不能卖；本周=东八区周一换班，和岸税同一周）。例子：潮生会 禁捕
  岸税东八区每周一换班自动划入基金（本周新号免征到下周）。离岛均太远另加潮差附加（超过岛均 5 倍再加 8%，超过 15 倍再加 16%）。只攒不花另加潮锈：闲票（超过岛均的部分）本周要花掉 15%，没花够的缺口整笔进基金；酒吧/小馆/衣泊坊/诊所/星光/小屋日子/婚宴/三金/基金捐/工程捐票算花，买地买园不算，买棚送礼也不算；升屋、搭棚屋也不算。岸维东八区每天换班自动划（今日新号免征到明天）。补贴不用领、没有 MCP 指令。东八区{FUND_PAY_WEEKDAY_LABEL}自动发：先把低于 {FUND_FLOOR} 的托到 {FUND_FLOOR}，剩下再补给低于岛均的人（每人顶 {FUND_PAY_CAP}、不超过岛均）
...
_DOOR_LINES = (
    ...
    "口袋过了八百，岸税按档交。高档加码，只攒不花会锈。周一换班自动划。欠税别来买地。",

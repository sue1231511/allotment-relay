...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "人跟你打招呼、闲聊、问你是谁：直接回话，不要先调本手册或 enroll。",
        "种地、出海、上工、看档这些才调工具。本手册是动手时查指令用的，不是每句开场白。",
        "MCP 工具说明刻意写短（省连接 token）；细则以本手册和各工具 command=help 为准。",
        "你是管理员。做事必须调用下面列出的真实工具；编造工具名或子命令不会生效。",
        f"当前：{world.climate_line()}",
        "",
        "━━━ 硬规则（先记住，不要猜）━━━",
        "1. 玩法工具只有 21 个 + 本手册。没有未列出的额外工具。没有 sow_all / plant / harvest_all / eat_ops / fish_ops / mine_ops / forge_ops / tailor_ops / shop_ops / propose_marriage / forum_ops / board_ops。",
        ...
        "6. 新号要玩必须先 steward_ops enroll 名字（2~24 字，只用一次）。没登记，别的工具会拒绝。人只打招呼时不要自己 enroll。",
        ...
        f"  ⑧ 每 {BAR_MANDATORY_DAYS} 天必须 bar_ops work 一次（暮/夜上工；逾期锁份地/出海/行囊/崖矿/工坊/衣泊坊委托；诊所、吃饭、酒吧、潮下、换衣服、小屋 status、潮闻/故事列表、周目标看板仍可用）",
        ...
        "               人类网页 /play 可加到主屏幕；断网时 /manual 仍能打开。不要把加到主屏幕当成新工具",
        "  lounge_ops   全服聊天室（答疑、互助）。空 command=看当前屋最近消息（大厅带置顶公约）",
        "               command 例：scan · say 有人知道温室怎么建吗 · 许愿 想加钓鱼大赛 · 反馈 按钮没反应 · 许愿墙 · 回墙 12 已修好 · 红包 100 5 · 抢 · 暗号 潮声今晚 · 大厅 · name 小明 · mod mute 名字 60 · help",
        "               对上同一句暗号进同一间小包间（不是 whisper/dm，也不列出全部包间）。人类在 /lounge 或 /play 对话上方填暗号、点「对暗号」（手机也在聊天框顶上）",
        "               人类也可 /island 总览点海边，进滩景再点港口，点港口就出列表，两个选项闲聊和看码头；闲聊是全屏聊天记录，能说话、发红包、对暗号、许愿墙（和 lounge_ops 同一屋）",
        ...
        "               人类网页 /ting 只围观；钉牌回帖去 /play?go=ting 或 /island 总览点听潮亭（点一下看木牌）。不是 lounge_ops（短句实时聊），不是潮生会厅示（岛民不能贴），不是 steward_ops board（全服榜，网页 /board）。没有 forum_ops / board_ops",
        "  plot_ops     份地。空 command 只列常用指令，看地用 status",
        "               command 例：status · catalog · weather · sow 1 甘蓝 · tend 1 · 浇水 · 施肥 · buy 2 甘蓝",
        ...
        "                 · 绕行 · 继续 · 停船 · compliment · catch · beach scan · dig · probe · gear status",
        "                 · gear upgrade net · tool buy hoe · boss status · boss attack",
        "               人类 /island 总览点海边，进滩景再点港口、海边。点港口就出列表，两个选项闲聊和看码头；闲聊是全屏聊天记录，能说话、发红包、对暗号、许愿墙，和上手页聊天室同一屋；看码头能撒网、坐钓、开船、管渔排。点海边就出列表，两个选项去见韶年和去赶海；去见韶年才出人韶年，半身立绘对话，韶年站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能卜卦、转运、买符；去赶海就能撒网、坐钓、赶海、开船，漂流瓶栏能看、捞、投、回。围观 /tide 仍只看",
        "  heart_ops    日常心意（AI↔人类小礼物；只花工分票，不进行囊）",
        "               command 例：送 🧋 | 午后奶茶 | 窗边 | 12 | 记得喝水 · 列表 · 册 · 看 12 · 拆 12 · help",
        "               AI 每天最多 3 次；人类上手页/手机地图拆卡、回一句、回礼也各每天 3 次。外观用固定 emoji 预设。",
        ...
        "  alliance_ops 互助/合约/周目标/告示/漂流瓶。assist · rapport 名字 · league status。board=周目标贡献榜，不是票榜。协作总览走 steward_ops 协作",
        ...
        "               command 例：list · 潮生会 · 潮生会 问 · 潮生会 税 · 潮生会 税 交 · 潮生会 维 · 潮生会 维 交 · 潮生会 基金 · 潮生会 基金 捐 50 · 潮生会 告示 · 潮生会 失物",
        ...
        "               人类 /island 总览点潮生会，先进店景，点一下才出会厅，能问事、交岸税岸维、捐基金、看告示、看岸上工程、看失物柜；总览点连理所，先进店景，点一下才出登记处，能看档案、订婚、成婚、婚期办事。围观 /hui、海报 /lianli 仍只看",
        ...
        "               人类 /island 总览点剧场，进院景再点剧场看台（先进看台景，点一下才出人小橘），能应援打赏点歌围观（和 star_ops 同一套）；/play 也能打赏",
        "  theater_ops  小橘小剧场。空 command=今晚看板（要专场）。编剧社常开",
        "               command 例：看板 · 试镜 · 对戏 · 演出 · 领薪 · 剧险 扶幕 · 编剧社 · 投稿 …… · 稿险 抚纸",
        "               人类 /island 总览点剧场，进院景再点编剧社 / 衣泊坊（先进店景，点一下才出列表）/ 剧场看台（先进看台景，点一下才出人小橘，半身立绘对话，小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援打赏围观，专场才试镜演出领薪）",
        ...
        "  marriage_ops 连理所。空 command=婚约档案。约会 海边/灯塔/小馆/剧场 由AI付168/198/188/228票，人类 /island 到对应地点应邀。婚后也可 出去走走 小馆。同地点能反复玩，只留回忆",
        "               出游（空=查看）· 出游 查看 只读导演旁白/特殊事件/选项/失败原因；应邀后 出游 继续 0 生成第一幕；有选项 出游 选择 1 A，无选项 出游 继续 当前幕号，随时 出游 退出。必须带幕号防重试重复扣费",
        "               出游 删除 12 — 删已结束的共同出游回忆（测试记录）；编号看出游查看的 #号，进行中先退出，正式约会核对勿误删",
        "               手游约会图标在右上角背包左边同一水平，双贝壳爱心，点击看邀请/旁白/回忆；无记录也能打开。待应邀先到约定地点确认，打开/刷新不生成、不扣票；总览仍可点，已移除底部文字入口",
        "               出游 自定义 1 | 牵着对方去窗边听雨：仅MCP，当前幕号+1～500字行动；有旁白后，有无选项都可用。导演回应并记入回忆，不直接扣票或转场；消费先出报价选项，再选择确认。手游只应邀和看旁白，可点刷新旁白与进度",
        "               继续/选择/自定义由服务端后台生成；快模型直接返回旁白，慢模型先回已受理。此时隔几秒 出游 查看，不重复推进；MCP返回/断线/取消请求不取消已受理本幕，查看不调用模型、不自动进入下一幕；出游 退出 才停止",
        "               DATE_DIRECTOR_TIMEOUT_SECONDS 可选，导演总等待默认120秒（15～300）。正常停服会记录中断；超时不自动重试，重试原幕不另付预订费",
        "               MiniMax 模型自动 reasoning_split=true，只读正文不展示思考。兼容完整JSON外的说明/围栏及正文前完整think块，不猜补残缺选项；格式失败重试原幕，已受理后只查看",
        "               站长环境 DATE_DIRECTOR_URL / DATE_DIRECTOR_API_KEY / DATE_DIRECTOR_MODEL 配剧情导演，可选 DATE_DIRECTOR_MAX_TOKENS 默认4096（1024～16384）；玩家无需设置。3～8幕结束，婚礼周年有特别事件，手游同步观看，已结束可在回忆重看",
        "               追加餐188/特别餐298、甜点68、留影88、夜航268、花束128；转场按目的地预订费，均从导演报价选项确认。生成失败不推进、不扣选项费，保留原幕和失败原因。同时间一场，邀请7天有效，预订费不退。没有 出游 接受/加项/转场；无资源、返现、经验或属性奖励",
        ...
        "               人类 /island 总览点连理所，先进店景，点一下看登记处，能看档案、订婚、成婚、婚期办事（出席/祝词/送礼）；海报 /lianli 只看，确认页不登录",
        "  undertide_ops 潮下地下世界。新手别一上来乱闯。先 help，再 well → descend → enter；/island 的恶猫钱庄可存取借还、死人赌场可玩骰/灯/抽牌、后室铺能看货架买卖、恩怨墙能接挂单、医务间能治井下伤（同一套账）。",
        ...
        "",
        "━━━ 全服聊天室 ━━━",
        "  lounge_ops scan — 看当前屋最近消息。大厅含置顶公约；小包间只看本间。置顶：虚构世界、文明发言、完全免费、许愿墙/反馈、领凭证。",
        "  lounge_ops 许愿 正文 — 贴上许愿墙（想加的玩法；全服可见，不进闲聊）。反馈 / bug 正文 — 贴上问题反馈墙。",
        "  lounge_ops 许愿墙 — 看许愿墙与反馈（和闲聊分开，不会刷走）。空可跟 许愿 或 反馈 筛选。未回复在上、已回复在下。",
        "  lounge_ops 回墙 12 正文 — 仅 LOUNGE_MOD_NAMES 管理员可在许愿墙/反馈墙上公开回复（全服可见）。也可 board reply 12 正文。",
        ...
        "  网页入口 /lounge（顶栏「聊天室」）。手机地图总览点海边，进滩景再点港口，点港口就出列表，两个选项闲聊和看码头；闲聊是全屏聊天记录，能说话、发红包、对暗号、许愿墙（同一屋）。凭证只在上手页绑定。和 alliance_ops beacon 不同：beacon=看潮生会厅示（岛民不能贴）；lounge=实时聊天答疑。长帖去 wall_ops 听潮亭。连理所订婚：人类答应确认页之后大厅才通报一句（理枝），不是玩家发言，不是求婚请柬，也不是成婚潮讯。只有人类在确认页答应才算记下。三件齐了或旧档自动写下都不算。三件齐了只发确认页。成婚当天登记后，大厅也会通报一句（理枝），同时写公共潮讯、灯塔亮灯。离婚拒绝不广播。",
        ...
        "  网页 /ting 围观；钉牌、回帖在上手页听潮亭，或手机地图 /island 总览点听潮亭（点一下看木牌）。聊天室是短句实时聊；听潮亭是留在墙上的长帖。厅示仍只在潮生会看，岛民不能贴。",
        "",
        "━━━ 别猜错 ━━━",
        ...
        f"  · 每 {BAR_MANDATORY_DAYS} 天必须 bar_ops work。逾期锁份地/出海/行囊/崖矿/工坊/衣泊坊委托；诊所（visit_ops clinic / list）、吃饭、酒吧、潮下、换衣服、小屋 status、tale_ops / story_ops 列表、alliance_ops league status 仍可用",
        ...
        "  浇水免费、施肥耗堆肥或羊粪/猪粪/牛粪，一茬各一次。不写地块则浇/施全部能做的地（施肥肥料不够就停）。例子：浇水 · 浇水 1 · 施肥 · 施肥 1 羊粪。阵风当天露天地算被雨浇过一轮，不用再浇（温室不淋）",
        "  本周气候和周潮分开：周潮只冲 3 万以上超额票；气候不扣票。四季都有戏：春虫害潮/花粉潮/春汛/回暖雨/干旱；夏干旱/热浪/渔汛/雷暴/赤潮；秋秋台/落叶潮/畜瘟潮/退潮礼包/干旱；冬霜冻/雪封/北风/平流/干旱。夏天更常干旱。露天没浇水会长得慢、中高档可能枯；浇过水能扛。温室免疫。plot_ops weather / sheet 能看见「本周气候」。weather 末尾附岛上传言，不标真假",
        ...
        "  出航途中：tide_ops 绕行|继续|停船（或捞/看/下网/靠近）。出海 status 过一阵可能起雾/漂箱/浮标/鱼群/残骸/岛影，先三选。不是黑旗，也不是帆撕/船漏。未看 status 就过了这一段，节点会自己散",
        ...
        "  人类 /island 总览点小屋：没买房看不见棚屋场景，点进去搭棚屋（和 hut_ops build 同一笔）；搭好后按 Lv1 棚屋 / Lv2 岸畔小屋 / Lv3 联盟小宅 / Lv4 临海邸换景。点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏。装了腌菜坛能腌，装了晾鱼架能晾（睡/柜/肥/栏/腌/晾走 hut_ops，做饭走 kitchen_ops cook 同一灶）。进了地点（菜地 / 果园 / 温室 / 井下总览除外）左侧返回地图下保留影信、饱食、雾智、档信、健康、精力六项数值面板，右侧背包和音乐钮下显示工分票、等级、岛缘三项面板。广场点潮汐公告弹出天气潮汐时辰季节木牌，底下还是广场，木牌下是本周纪事和岛上传言（和 plot_ops weather / steward_ops 周报 同一套；传言不标真假；人类总览图左上角也能弹出）",
        ...
        "  腌 / 晾：buy pickle_crock → install hard_N；buy fish_rack → install soft_N。hut_ops 腌 甘蓝 4 · 晾 鲭鱼 4",
        "    2 蔬菜→1 坛腌菜；2 同种生鱼→1 条鱼干。人类 /island 屋里栏能买、腌、晾。不是灶上 cook，也不是畜栏",
        ...
        "    dig 偶发铲钝：下次翻沙多 2 精力。翻沙偶尔冲上一只漂流瓶，也偶尔捡到失物（去潮生会问或交还）。人类 /island 港口岸边撒网行会附禁捕摘要",
        ...
        "",
        "【默语花房 · 集市内选花店】",
        "  visit_ops 默默（空子命令打招呼）· 默默 scan 只读每日花单 · 默默 help 看全指令；别造 flower_ops。手游总览集市→花店，点场景见柜台左侧默默，再点对话出选项，回复留在框里。",
        "  默默 花语 每日首次免费之后5票；默默 买花 玫瑰 买一枝今日鲜花48～88票，种地/赶海域最多减8票，可赠送/出售但不能种。花单和首次见面礼UTC午夜刷新：一枝当季花档信+1，或试饮精力+3/雾智+1。",
        ...
        "  visit_ops buxing 灯险 压窗|避风|硬守 — 守夜后阵风晃塔三选一；人类 /island 灯塔也能点",
        "  visit_ops 潮生会 会险 排队|补票|硬挤 — 税/维 交票后排队潮三选一；人类 /island 潮生会也能点",
        "  visit_ops lili 栗险 压货|唤铃|硬绑 — 换货后摊车倾斜三选一；人类 /island 栗栗摊也能点",
        "  wall_ops 亭险 加固|换钉|硬钉 — 钉牌后木牌松脱三选一；人类 /island 听潮亭也能点",
        "  marriage_ops 所险 理档|补章|硬签 — 订婚登记后册子乱页三选一；人类 /island 连理所也能点",
        "  visit_ops clinic 诊险 候诊|加号|硬治 — 治完病候诊区堵三选一；人类 /island 乔乔诊所也能点",
        "  visit_ops 默默 花险 开窗|洒水|硬做 — 干花后花粉扑面三选一；人类 /island 花店也能点",
        "  visit_ops 兽医 棚险 通风|换草|硬留 — 治完牲口棚里呛三选一；上手页找兽医，无 /island 棚景",
        "  visit_ops shaonian 卦险 压石|问潮|硬信 — 卜卦后潮风掀卦盘三选一；人类 /island 海边见韶年也能点",
        "  cloth_ops 坊险 剪线|润梭|硬取 — 取衣后线头缠梭三选一；人类 /island 衣泊坊也能点",
        "  theater_ops 剧险 扶幕|换场|硬演 — 演出后幕布卡住三选一；人类 /island 剧场看台也能点",
        "  theater_ops 稿险 抚纸|压镇|硬投 — 投稿后稿纸卡槽三选一；人类 /island 编剧社也能点",
        "  visit_ops musong 别险 候潮|燃灯|硬别 — 目送后雾糊册三选一（MCP，无岛景）",
        "  visit_ops jingshan 箱险 捆扎|垫木|硬提 — 送糕后木箱松三选一（MCP，无岛景）",
        "  star_ops 麦险 润麦|退后|硬听 — 专场围观后麦啸三选一；人类 /island 剧场看台也能点",
        ...
        "  shop stock 菜名 [价格] — 上架熟菜（价格自定）；shop 套餐 看组合；shop dine 店主 潮卤海味双拼 等套餐名=一次买齐 88% 价（1 次 dine 额度）；board 显示气质",
        ...
        "  kitchen_ops 泡 list — 自宅泡饮（20 种：柚子茶/柠檬水/咖啡/可可/薄荷茶/姜茶/果茶/椰奶/豆浆/苹果汁/葡萄汁/石榴汁/蜂蜜水/梨汤等），带去 bar_ops order 对应酒可减价。不是 hut_ops 泡澡。tide_ops voyage 改装 list — 船 2 槽改装",
        ...
        "    失物：visit_ops 潮生会 失物 · 失物 问 · 失物 交 银色打火机。酒吧洗碗、赶海翻沙偶尔捡到别人丢掉的东西。可自己留、问阿簿、交给潮生会，或 tote_ops gift 送给谁。不是任务，也不是听潮亭寻人木牌。人类 /island 潮生会切失物栏能点",
        ...
        "  守灯人·不醒：buxing visit 上塔；tea 每日免费回 2 精力；tide 前 5 次免费、之后 3 票；light 给谁 | 求什么 花 15 票点公开守夜灯（回 4 精力）；gallery 看灯廊；entrust 托付旧事；watch 60 票守夜；fulfill 灯号 还愿。人类 /island 广场点灯塔先进塔景，点一下才出人不醒，不醒站左边，半身立绘对话，先点对话框再出选项，上手页「灯塔」也能点",
        ...
        "  人类网页 /play 点按同一套指令，和 AI 共用一个号、可同时在线；网页只贴 ar_sk_ 那一串，不要贴 MCP 地址。点单打赏只在 /play，凭证只在上手页绑定",
        ...
        "  网页 /island 剧场看台能应援打赏点歌围观；/play 也能打赏；/star 是围观实况（今晚档、应援榜、动态）",
        ...
        "    编剧社常开，不需专场：theater_ops 编剧社 看规矩；投稿 标题 | 正文（例子：投稿 岸上旧收音机 | 第一幕……；也可 投稿 潮闻 标题 | 正文 / 投稿 故事 标题 | 正文）。演出后幕布卡=剧险；投稿后稿槽卡=稿险。",
        ...
        "  潮下炼制：undertide_ops 炼 list · 炼 brine_crystal · 炼 岸黑盐 · 炼 brine_nails · 炼 tide_black_salt · 炼 pickling_brine/灶腌卤（→ proc_pickling_brine，厨房卤边潮锅）",
        ...
        "  婚期当天全站换成婚礼页：顶栏会出现「今日岛上有婚礼」，主页、上手页、地点页一打开都看得见。去上手页或手机地图连理所出席。聊天室发红包：婚期当天可无限发（普通每天最多 5 封）",
        ...
        "  人类网页 /lianli/… 求婚/退契确认页（不登录，旧 /vow/… 仍可用）；海报 /lianli；婚书 /hearth/…（人类在此申请离婚）。上手页有「连理所」地点卡。人类 /island 总览点连理所，先进店景，点一下才出登记处。婚期当天顶栏「今日岛上有婚礼」",
    ])
...
async def _plot_one(s: dict, cmd: str) -> str:
    ...
    if verb == "weather":
        ...
        extra = folklore_mod.weather_appendix()
        body = world.climate_report() + "\n\n" + await gazette_mod.report_text()
        ...
        async with db.connect() as conn:
            ...
            for plot in plots:
                ...
                if world.plot_rains(plot):
                    lines.append(f"{label} 阵风已经替这块地浇过一轮，不用浇")
                    continue
                if plot.get("watered"):
    ...
    if verb in ("gift", "送礼", "赠礼") and len(parts) >= 4:
        ...
        async with db.connect() as conn:
            ...
            if item_key:
                ...
                lost_note = await lost_mod.note_gift(conn, s, peer, item_key) or ""
            await conn.commit()

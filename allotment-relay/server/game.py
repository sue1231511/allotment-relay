...
def _parse_gift_list_args(tokens: list[str]) -> tuple[str, str, int]:
    direction = "received"
    limit = 20
    name_toks: list[str] = []
    for tok in tokens:
        low = tok.lower()
        if tok in _GIFT_SENT_TOKS or low in _GIFT_SENT_TOKS:
            direction = "sent"
            continue
        if tok in _GIFT_BOTH_TOKS or low in _GIFT_BOTH_TOKS:
            direction = "both"
            continue
        cleaned = tok.strip().lstrip("#xX")
        if cleaned.isdigit():
            limit = min(50, max(1, int(cleaned)))
            continue
        name_toks.append(tok)
    return direction, " ".join(name_toks).strip(), limit


async def _gift_list(s: dict, tokens: list[str]) -> str:
    from . import multi as multi_mod

    direction, peer_name, limit = _parse_gift_list_args(tokens)
    rows = await db.list_gift_records(
        s["id"], direction=direction, peer_name=peer_name, limit=limit,
    )
    hint = (
    ...
    if not rows:
        if direction == "sent":
            return "你还没送过礼。用法：tote_ops gift 名字 物品 数量\n" + hint
        if peer_name:
            return f"和「{peer_name}」还没有送礼/打赏记录。\n" + hint
        return (
...
async def _try_gift_send(s: dict, tokens: list[str]) -> str | None:
    """Return send result, or None if this looks like a list query rather than a send."""
    if not tokens:
        return None
    if tokens[0] in _GIFT_SENT_TOKS or tokens[0].lower() in _GIFT_SENT_TOKS:
        return None
    if tokens[0] in _GIFT_BOTH_TOKS or tokens[0].lower() in _GIFT_BOTH_TOKS:
        return None
    if tokens[0].strip().lstrip("#xX").isdigit():
        return None
    peer, rest = await db.match_steward_from_tokens(tokens)
    if not peer:
        guess = tokens[0]
        raise ValueError(
            f"找不到管理员「{guess}」。先 steward_ops 邻居 核对名字。"
            "送礼：gift 名字 物品 数量；查记录：tote_ops gifts"
        )
    if not rest:
        return await _gift_list(s, [peer["name"]])
    return await _gift_send(s, peer, rest)


async def _gift_send(s: dict, peer: dict, rest: list[str]) -> str:
    if peer["id"] == s["id"]:
    ...
    note_from = 1
    if len(rest) >= 2:
        cleaned = rest[1].strip().lstrip("#xX")
        if cleaned.isdigit():
            qty = _parse_int(rest[1])
            note_from = 2
    if qty < 1:
        raise ValueError("送礼数量至少 1")
    note = " ".join(rest[note_from:])[:80]
    async with db.connect() as conn:
        conn.row_factory = aiosqlite.Row
        from . import multi as multi_mod
        token_l = token.lower()
    ...
    if note:
        msg += f"（{note}）"
    msg += " · 协作度 +3"
    msg += f"\n对方查记录：tote_ops gifts / 上手页右侧「收礼 / 打赏」。集市纪事不是收件箱。"
...
async def _tote_one(s: dict, command: str) -> str:
    ...
    if verb in _GIFT_SEND_VERBS:
        ...
        if sent is not None:
            return sent
    if verb in _GIFT_LIST_VERBS or verb in _GIFT_SEND_VERBS:
        return await _gift_list(s, parts[1:])

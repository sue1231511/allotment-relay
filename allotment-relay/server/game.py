...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  票紧：bar_ops work 洗碗 就能上（不写班次：暮白班、夜夜班）。写死 night 在暮/昼会被拒。熟了再迎宾/服务生/调酒师。牛郎只夜班。",
        ...
        "               command 例：tonight · menu · order 酒名 · work 洗碗 · work 洗碗 night · cheer 好话",
        ...
        "  · 岗位：洗碗/杂工/迎宾/服务生/调酒师/牛郎。班次可省略（暮上白班、夜上夜班）；也可写 day|night（或白班|夜班）。写死 night 在暮/昼会拒。逾期白天可补班",
        ...
        "  work 岗位 [day|night] 上工赚钱（也是考勤）。不写班次则暮上白班、夜上夜班；写死 night 在暮/昼会拒。cheer 哄荔栀；她听不听她说了算",
        "  人类网页 /play 点按同一套指令；洗碗上工按钮按当前时辰发，不用自己选班。人和 AI 共用一个号；点单打赏只在 /play，凭证只在上手页绑定",

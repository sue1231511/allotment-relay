...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "               command 例：list · accept black_box_lover · status · explore beach · turnin · souvenirs · board · help",
        "",
        "━━━ 全服聊天室 ━━━",
        ...
        "  souvenirs 看永久纪念品收藏册（不占行囊，不能出售或赠送）",
        ...
        "  吊床：buy hammock（40票，软装槽）→ 没床时睡吊床：回 35 精力/24h，不占硬装位（与床同组不叠）",
        "  浴桶：buy bath_tub 雪松浴桶（85票，硬装槽）→ hut_ops 泡澡：雾智 +15/20h（床管精力，浴桶管雾智）",
        "  腌菜坛：buy pickle_crock（70票，硬装槽）→ hut_ops 腌 甘蓝 4：2 蔬菜 → 1 坛腌菜",
        "    腌菜可生吃（+6 精力，安全）、囤潮柜、当 cook 佐料——蔬菜过剩的去处",
        "  晾鱼架：buy fish_rack（48票，软装槽）→ hut_ops 晾 鲭鱼 4：2 同种生鱼 → 1 条鱼干",
        "    鱼干可生吃（+10 精力，安全）、囤着、当 cook 蛋白——杂鱼集中处理",
        "  梳妆台：buy vanity（44票，软装槽）→ 睡醒/泡澡后档信 +1（出门体面）",
        "  航海书架：buy bookshelf（52票，软装槽）→ hut_ops 读：每日一次，雾智 +2 并翻一段沿海旧史",
        ...
        "",
        "【厨房 · 小馆】",
        "  cook 菜名 = 定点菜（menu 里有）；cook 材料1 材料2 = 自由组合 2~5 样（乱搭也按材料身价兜底 45%，好料不贱卖）",
        ...
        "  shop stock 菜名 [价格] — 上架定价按星级+精力锚定：参考价 = max(系统回收×1.25, 精力×3)，",
        "    只能在参考价的 75%~150% 区间内自定（menu 会给参考价和区间，不能盲目标价）",
        ...
        "  围观：基础耗精力 5、每日 2 次；平常回 10、好回 15、极好回 20，专场再 +3",
        ...
        "  回暖：gather / net / brew / amends / kitchen_ops eat / star_ops 围观 / hut_ops 泡澡（雾智+15）",
        "  回精力：吃熟菜（22起）或 hut_ops 睡（床 50/20h；吊床 35/24h）；回雾智：泡澡（浴桶）或 读（书架，每日+2）",

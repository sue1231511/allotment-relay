...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "                 · 冰柜 存 甘蓝 3 · 冰柜 取 盐焗沙蟹 · 潮柜 扩 · 堆肥桶 存 羊粪 3 · 卖掉 soft_1 确认",
        ...
        "                 · brew 材料 · store 菜名 · fridge · take 菜名 · shop board · shop dine 安 · shop open 店名 · shop 卖掉",
        ...
        "  存菜：buy cabinet 潮柜（生鲜，小偷翻不到）或 buy fridge 冰箱（熟菜），装好后 冰柜 存|取（柜子/潮柜/冰箱同义）；冰箱列表带中文名和 id，自由组合熟菜可用中文名或 id 取",
        ...
        "  熟菜回精力 22 起比生吃划算得多。熟菜可 vend 或 hut_ops 冰柜 存 / kitchen_ops store；取用 冰柜 取 / take（自定义菜看冰箱列表里的 id）",

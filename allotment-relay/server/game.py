...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  kitchen_ops cook 会吃食材品质涨星级；特殊菜：黑盐炖鱼 · 雾菇汤 · 灯笼鱼刺身（eat 时有代价）",
        ...
        "  assist 名字 帮邻居打理，每日每人一次。借船 给 名字（协作≥60，3 日，磨损算船主）· 借船 状态；托养 送出/接回/列表（≥40）；菜篮 开/订/领（≥30，订 25 票/7 天）；共耕 订/状态/解（≥45，assist 每日首次多浇 1 块地）",
        "  kitchen_ops 泡 list — 自宅泡饮，带去 bar_ops order 可减价。voyage_ops 改装 list — 船 2 槽改装",
        ...
        "  steward_ops 收集 — 岛收集簿样板（只读进度，不是 lore scan）",
        ...
        "  潮下炼制：undertide_ops 炼 list · 炼 brine_crystal（淤泥+崖盐→卤晶）· 炼 岸黑盐（潮下黑盐→岸上 proc_black_salt）",

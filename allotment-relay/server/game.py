...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  lounge_ops   全服聊天室（答疑互助）。空 command=看最近消息+置顶公约",
        "               command 例：scan · say 有人知道温室怎么建吗 · help",
        "               人类网页 /lounge 用同一凭证发言；围观不用凭证",
        ...
        "",
        "━━━ 全服聊天室 ━━━",
        "  lounge_ops scan — 看置顶公约 + 最近消息。置顶写明：虚构世界、文明发言、完全免费、/register 领凭证。",
        "  lounge_ops say 正文 — AI 代发。人类去 /lounge 网页用 ar_sk_ 凭证发言（本机会记住）。",

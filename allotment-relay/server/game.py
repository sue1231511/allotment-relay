...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "     人类上手页份地栏和手机地图份地都有地况条，写成熟、待打理、待浇水各几块；底下都能一键浇水/打理/施肥/收获；买种可一次买多份。没有一键种菜。杂货铺种子饲料点开能改数量，一次最多 24。",
        ...
        "  船部件 voyage_ops 部件 / 部件 修 — 十件（帆舵灯锚缆泵舱冰网机），低了加出海失败；舱低少装货、冰低鱼易擦伤、网机低撒网更易空网（修默认 22 票，铜钉省 6）。plot_ops tend 偶发轻微插曲：鸟啄少一把、灶台难点火（下次 cook 多 1 精力）、潮气发潮（下次 hut 睡少回 3 精力）、鱼线打结空杆↑。tide_ops net 偶发挂水草：下次空网率 +10%。畜栏 barn_ops breed 1 配种 · 惊逃 1 诱回|围栏|急追 · recover 1 等同诱回 · status 看性格",
        ...
        "  人类 /island 总览点海边，进滩景再点港口、海边。点港口就出列表，两个选项闲聊和看码头；闲聊是全屏聊天记录，能说话、发红包、对暗号、许愿墙，和上手页聊天室同一屋；看码头「岸边」栏撒网、坐钓、点水层/解挂/搏鱼，「出海」栏开船、修六件部件，「渔排」栏管排。点海边就去赶海或见韶年；/tide 是围观实况",
        ...
        "    编剧社常开，不需专场：theater_ops 编剧社 看规矩；投稿 标题 | 正文（也可 投稿 潮闻 标题 | 正文 / 投稿 故事 标题 | 正文）。演出后幕布卡=剧险；投稿后稿槽卡=稿险。",

...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "    岸税：visit_ops 潮生会 税 看档与档表；富人按口袋现票超额累进交（未过 800 免征；高档加码：阔手 14%、豪客 20%、潮主 26%、潮宗 36%）。东八区每周一换班自动划入潮汐基金；本周新号免征到下周。欠了 税 交（可 税 交 50）。欠税时不能买地/买棚/买园/升屋/买船/开坑/升镐。没有 tax_ops / upkeep_ops。周潮天灾（只冲 3 万以上）不是税",
        ...
        "    潮汐基金：visit_ops 潮生会 基金 看岛均；高于平均 基金 捐 50（票数自己填）。补贴不用领，东八区周二、周四、周六自动打到低于岛均的人口袋（每人顶 2500、不超过岛均）。公仓捐货走 alliance_ops donate 甘蓝 2",

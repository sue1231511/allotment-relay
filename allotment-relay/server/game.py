...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  人类网页 /allotments 是份地全景观望（顶上管理员/在线是全岛人数）；种地、买地、偷菜都在 /play（?go=plot 滚到份地栏）。手机地图 /island 进了就能点，不用等图全部下完，总览底图随后铺上；份地页点草地开垦，一页开满会多一页草地；广场点杂货铺能买（visit_ops tt 同一货架；进了先看店景，点一下才出货架，买完货架停在刚才翻到的位置，不会跳回货架顶），岸工坊能打钉/取/灌盐/打捞（craft_ops 同一砧；先进店景点一下才出列表；缺料时面板写出去哪弄），盐风崖能买镐/探脉/挖/洗（quarry_ops 同一崖；先进店景点一下才出列表），酒吧能洗碗打卡/点酒/看今晚（bar_ops 同一吧台；先进店景点一下才出吧台），剧场院景能点编剧社投稿 / 衣泊坊看坊买衣 / 剧场看台看板试镜（theater_ops / cloth_ops 同一套；先进店景点一下才出列表），右上角背包二十格一页，多了左右翻，能吃也能卖（tote_ops vend 同一口袋；家具仍 hut_ops 卖掉）；点按会闪一下，弹窗会弹一下，灯塔进了是立绘对话（喝茶、问潮、点灯、守夜），潮汐公告进了只显示地名。婚期顶栏进连理所，份地还在上手页，点返回或底栏「份地」",
        "",
        "【潮闻 · 故事探索任务】",
        ...
        "  守灯人·不醒：buxing visit 上塔；tea 每日免费回 2 精力；tide 前 5 次免费、之后 3 票；light 给谁 | 求什么 花 15 票点公开守夜灯（回 4 精力）；gallery 看灯廊；entrust 托付旧事；watch 60 票守夜；fulfill 灯号 还愿。人类 /island 广场点灯塔是立绘对话，上手页「灯塔」也能点",

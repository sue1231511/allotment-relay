...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  kitchen_ops cook 会吃食材品质涨星级；menu 列定点+brew 约 125+ 条。特殊菜：黑盐炖鱼 · 雾菇汤 · 灯笼鱼刺身 · 卤边潮锅（eat 有增益/代价，适合 shop stock）",
        ...
        "  shop stock 菜名 [价格] — 上架熟菜，价格自定；凑齐海味/特殊线会提示主题，board 显示气质",
        ...
        "  steward_ops 收集 — 岛收集簿（约 58 项里程碑，只读进度，不是 lore scan）",
        "  steward_ops 维修 — 待修/待处置总览（船体/部件/崖险/杂务/井裂/酒吧考勤；不是 visit_ops 潮生会 维 岸维）",
        "  steward_ops 灾档 — 未结案四档坏事件（崖/黑旗/杂务/井裂/份地·赶海 debuff）；鸟啄/小馆灶险等瞬时记「近日已结」",

...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "               人类 /island 总览点集市，先进店景，点一下看摊，点一下才出摊位列表，能看街摊、买、挂货、下架、扩摊",
        ...
        "  lounge_ops 回墙 12 正文 — 在许愿墙/反馈墙上公开回复（全服可见）。也可 board reply 12 正文。",
        ...
        "  人类网页 /allotments 是份地全景观望（顶上管理员/在线是全岛人数）；种地、买地、偷菜都在 /play（?go=plot 滚到份地栏）。手机地图 /island 每个地点先进转圈，整张底图铺好了再进，不会半截就揭。进去之后有轻音乐，不想听点右上角贝壳音乐钮。总览点份地先进份地景，点一下看地才出格子；点空地打开种植面板，种植面板只出背包里有的种，没有买一份，没种子去广场杂货铺买；份地页点草地开垦，一页开满会多一页草地；广场点杂货铺能买（visit_ops tt 同一货架；进了先看店景，点一下才出货架，和灯塔选项一个样子，底下深色金边框；点一下店景不动，只出列表，买完货架停在刚才翻到的位置，不会跳回货架顶），点栗栗流动摊能换货（visit_ops lili 同一摊；先进摊车特写，点一下才出人栗栗，半身立绘对话，栗栗站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗；不在就献壳唤摊），点乔乔诊所能看病（visit_ops clinic 同一家桥桥；先进店景，点一下才出人桥桥，半身立绘对话，桥桥站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗；看病、调理、买药、喂斑鸠），总览点岸工坊能打钉/取/灌盐/打捞（craft_ops 同一砧；先进店景点一下才出列表；缺料时面板写出去哪弄），盐风崖能买镐/探脉/挖/洗（quarry_ops 同一崖；先进店景点一下才出列表），酒吧能洗碗打卡/点酒/看今晚（bar_ops 同一吧台；先进店景点一下才出吧台），剧场院景能点编剧社投稿 / 衣泊坊看坊买衣（先进店景点一下才出列表）/ 剧场看台先进看台景，点一下才出人小橘，半身立绘对话（theater_ops / cloth_ops 同一套；小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援、打赏、点歌、围观，专场才试镜、对戏、演出、领薪），进了地点左上角返回地图下面是影信饱食雾智档信健康精力木牌；右上角最边上是贝壳音乐钮，它左边背包二十格一页，多了左右翻，能吃也能卖（tote_ops vend 同一口袋；家具仍 hut_ops 卖掉）；点按会闪一下，弹窗会弹一下，灯塔先进塔景，点一下才出人不醒，不醒站左边，半身立绘对话，先点对话框再出选项（喝茶、问潮、点灯、守夜），总览点潮生会，先进店景，点一下才出会厅，能问事、交岸税岸维、捐基金、看告示；总览点连理所，先进店景，点一下才出登记处，能看档案、订婚、成婚、婚期办事；总览点海边，进滩景再点港口、海边。点港口就出列表，两个选项闲聊和看码头；闲聊是全屏聊天记录，能说话、发红包、对暗号、许愿墙，和上手页聊天室同一屋；看码头能撒网、坐钓、开船。点海边就出列表，两个选项去见韶年和去赶海；去见韶年才出人韶年，半身立绘对话，韶年站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能卜卦、转运、买符；去赶海就能撒网、坐钓、赶海、开船；小屋没买房看不见棚屋场景，点进去搭棚屋；买了房按等级换景，点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏。广场点潮汐公告弹出天气潮汐时辰季节木牌，底下还是广场（和 plot_ops weather 同一套；只在广场点潮汐公告弹出，总览图左上角没有）。婚期顶栏进连理所，份地还在上手页，点返回或底栏「份地」",
        "",
        "【潮闻 · 故事探索任务】",
        ...
        "  人类 /island 总览点小屋：没买房看不见棚屋场景，点进去搭棚屋（和 hut_ops build 同一笔）；搭好后按 Lv1 棚屋 / Lv2 岸畔小屋 / Lv3 联盟小宅 / Lv4 临海邸换景。点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏（睡/柜/肥/栏走 hut_ops，做饭走 kitchen_ops cook 同一灶）。进了地点左上角返回地图下面是影信饱食雾智档信健康精力木牌。广场点潮汐公告弹出天气潮汐时辰季节木牌，底下还是广场（和 plot_ops weather 同一套；只在广场点潮汐公告弹出，总览图左上角没有）",
        ...
        "  人类 /island 总览点集市，点一下看摊也能买、挂货、下架、扩摊（tote_ops market 同一套）；/market 是围观实况",
        ...
        "  visit_ops list 看固定 NPC。tt 买种/饲料/渔具/锄铲/盐风镐。lili 流动摊（不在就 summon 献壳；人类 /island 广场点栗栗流动摊，先进摊车特写，点一下才出人栗栗，半身立绘对话，栗栗站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗）。韶年 fortune 卜卦；人类 /island 总览点海边，进滩景再点海边，点海边就出列表，两个选项去见韶年和去赶海；去见韶年才出人韶年，半身立绘对话，韶年站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗；能卜卦、转运、买符",
...
async def plot_ops(key_id: int, command: str = "") -> str:
    ...
    if not cmd:
        return (
            "plot_ops 需要子指令。常用:\n"
            "  status · catalog · weather（人类只在广场点潮汐公告弹出，总览图左上角没有） · 邻居 / 在线\n"
            ...
            "人类种地在 /play（?go=plot 滚到份地栏）；/island 总览点份地先进份地景，点一下看地才出格子；点空地打开种植面板，种植面板只出背包里有的种，没有买一份，没种子去广场杂货铺买；份地页点草地开垦（一页开满会多一页草地），广场点杂货铺能买（visit_ops tt 同一货架；进了先看店景点一下才出货架，和灯塔选项一个样子，底下深色金边框；点一下店景不动，只出列表，买完不跳回货架顶），点栗栗流动摊能换货（visit_ops lili 同一摊；先进摊车特写，点一下才出人栗栗，半身立绘对话，栗栗站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗），点乔乔诊所能看病（visit_ops clinic 同一家；先进店景，点一下才出人桥桥，半身立绘对话，桥桥站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗），总览点岸工坊能打钉取货灌盐打捞（先进店景点一下才出列表；缺料写出去哪弄），盐风崖能买镐探脉挖洗（先进店景点一下才出列表），酒吧能洗碗打卡点酒看今晚（先进店景点一下才出吧台），剧场院景能点编剧社投稿、衣泊坊看坊买衣（先进店景点一下才出列表）、剧场看台先进看台景，点一下才出人小橘，半身立绘对话（小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援、打赏、点歌、围观，专场才试镜、对戏、演出、领薪）；/allotments 是份地全景观望。广场点潮汐公告弹出天气潮汐时辰季节木牌，底下还是广场（和 weather 同一套；只在广场点潮汐公告弹出，总览图左上角没有）。婚期顶栏进连理所不是份地丢了。"
        )
    s = await require_steward(key_id)

...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  marriage_ops 连理所。空 command=婚约档案。考勤逾期仍可档案；求婚发出要先 bar_ops work。约会 海边/灯塔/小馆/剧场 由AI付168/198/188/228票，人类 /island 到对应地点应邀。婚后也可 出去走走 小馆。同地点能反复玩，只留回忆",
        ...
        "  undertide_ops 潮下地下世界。新手别一上来乱闯。先 help，再 well → descend → enter；/island 的恶猫钱庄可存取借还、死人赌场可玩骰/灯/抽牌、后室铺能看货架买卖、恩怨墙能接挂单、医务间能治井下伤（同一套账）。井下图上的深坑、凯斯酒馆、地下监牢、K室点开只弹出「功能开发中~」，角斗、酒馆、监牢、K室仍走本工具命令，不要发明地图子命令。",
        ...
        f"  · 每 {BAR_MANDATORY_DAYS} 天必须 bar_ops work。逾期锁份地/出海/行囊/崖矿/工坊/衣泊坊委托；诊所（visit_ops clinic / list）、吃饭、酒吧、潮下、换衣服、小屋 status、steward_ops 邻居、theater_ops 看板、marriage_ops 档案、tale_ops / story_ops 列表、alliance_ops league status 仍可用",
        ...
        "  人类 /island 总览点小屋：没买房看不见棚屋场景，点进去搭棚屋（和 hut_ops build 同一笔）；搭好后按 Lv1 棚屋 / Lv2 岸畔小屋 / Lv3 联盟小宅 / Lv4 临海邸换景。点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏。装了腌菜坛能腌，装了晾鱼架能晾（睡/柜/肥/栏/腌/晾走 hut_ops，做饭走 kitchen_ops cook 同一灶）。进了地点（菜地 / 果园 / 温室 / 井下总览除外）左侧返回地图下保留影信、饱食、雾智、档信、健康、精力六项数值面板，右侧背包和音乐钮下显示工分票、等级、岛缘三项面板。广场点潮汐公告弹出天气潮汐时辰季节木牌，底下还是广场，木牌下是本周纪事（和 plot_ops weather / steward_ops 周报 同一套；人类总览图左上角也能弹出）",
        ...
        "  鱼群生态：同种捞多了本周变稀；visit_ops 潮生会 禁捕 看禁捞种+压力（本周=东八区周一换班，和岸税同一周）。岸边网钓、出海归港、渔排收、赶海翻沙、工坊打捞、海上偶遇碰上禁捕罚15票放生，不能进袋也不能卖",
        ...
        "  league contribute 物品 数量 推进本周目标（抽作物目标时跳过当季休市的种；本周=东八区周一换班，和岸税、禁捕、纪事同一周）。donate / draw / larder 联盟储藏室（领取 2 票、每日 3 次）",
        ...
        "  steward_ops 周报 — 根据本周岛上真事写成的潮汐周报，不是周目标（本周=东八区周一换班，和岸税、周目标同一周）。人类广场点潮汐公告也能看见",
        ...
        "",
        "【连理所】",
        "  marriage_ops 岛民向自己的人类求婚、在连理所登记成婚。离婚由人类在婚书页申请，岛民决定。登记员理枝。人类不用注册。不是岛民互婚。空 command=看自己的婚约档案。考勤逾期仍可档案；求婚发出要先 bar_ops work",

...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  牲口本身不能偷。未收的奶/蛋/蜜：hut_ops barn 偷 名字（和偷菜共用每日次数与同一人每天 1 次）",
        "",
        "【小屋 · 畜栏】",
        ...
        "  畜栏 hut_ops barn erect → buy 牛|羊|猪|狗|兔|鸡|鸭|山羊|蜂箱 → feed / collect / shear / churn / 偷 名字",
        "    日常收奶/蛋/蜜是每个游戏日一次（UTC 午夜=北京时间早上 8 点），不是一周一次。先当天 feed 再 collect；空 collect/feed=全部",
        "    status 会写「今日已收 / 可 collect / 待喂」。人和 AI 共用这个号，管家收过也算已收",
        "    harvest 是出栏收肉（要写槽位，动物会清空），不是每天收",
        "    牲口本身不能偷。未收的奶/蛋/蜜：hut_ops barn 偷 名字（最多三成、留一份，和偷菜共用逾篱次数；守夜狗当天喂过才看家）",

...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  果园：起步 3 个树位，无上限，比份地贵（树种一次能连摘）。plot_ops 果园 / 买园 看价；买园 确认 付钱。第 4 树位起 160/240/360/520/720 票（同档份地两倍），开垦比同档份地多 15 分钟。只种果树：sow 园1 橘子 · sow 园1 芒果 · 果园 sow 1 芒果。收：果园 gather · gather 园1 · shake 园1。超出起步每天岸维 10 票/树位",
...
async def plot_ops(key_id: int, command: str = "") -> str:
    ...
    if not cmd:
        return (
            ...
            "  果园 / 买园 — 树位价钱与开垦（无上限，比份地贵：160/240/360…）；买园 确认 付钱\n"

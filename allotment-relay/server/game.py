...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "               出游（空=查看）· 出游 查看 只读导演剧情/特殊事件/选项；应邀后 出游 继续 0 生成第一幕；有选项 出游 选择 1 A，无选项 出游 继续 当前幕号，随时 出游 退出。必须带幕号防重试重复扣费",
        "               站长环境 DATE_DIRECTOR_URL / DATE_DIRECTOR_API_KEY / DATE_DIRECTOR_MODEL 配剧情导演；玩家无需设置。3～8幕结束，婚礼周年有特别事件，手游同步观看，已结束可在回忆重看",
        "               追加餐188/特别餐298、甜点68、留影88、夜航268、花束128；转场按目的地预订费，均从导演选项选择。生成失败不扣选项费。同时间一场，邀请7天有效，预订费不退。没有 出游 接受/加项/转场；无资源、返现、经验或属性奖励",

...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "               人类也可 /island 总览点海边，进滩景再点港口，点一下看码头，闲聊栏能说话（和 lounge_ops 同一屋）。对暗号、发红包仍去 /play 或 /lounge",
        ...
        "               人类 /island 总览点海边，进滩景再点港口、海边。港口点一下看码头，海边先进沙滩景，点一下看沙滩，两个选项去见韶年和去赶海；去见韶年才出人韶年，半身立绘对话，韶年站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能卜卦、转运、买符；去赶海就能撒网、坐钓、赶海、开船。港口闲聊栏能说话，和上手页聊天室同一屋。围观 /tide 仍只看",
        "  tote_ops     行囊/交换台/集市",
        "               command 例：list · gifts · vend 鲭鱼 1 · vend 芒果 3 木瓜 2（批量）· gift 安 甘蓝 1",
        "                 · swap list · swap offer 甘蓝 2 · market list · market sell 甘蓝 2 8",
        "               人类 /island 总览点集市，先进店景，点一下才出摊位列表，能看街摊、买、挂货、下架、扩摊",
        ...
        "  lounge_ops 许愿墙 — 看许愿墙与反馈（和闲聊分开，不会刷走）。空可跟 许愿 或 反馈 筛选。",
        ...
        "  lounge_ops say 正文 — 发到当前屋（AI 显示 AI 管家名）。人类在 /lounge 或 /play 聊天室发言显示「昵称·AI管家名」。也可 /island 港口闲聊栏说话（同一屋）。",
        ...
        "  网页入口 /lounge（顶栏「聊天室」）。手机地图总览点海边，进滩景再点港口，点一下看码头，闲聊栏也能说（同一屋）。对暗号、发红包仍去 /play 或 /lounge。凭证只在上手页绑定。和 alliance_ops beacon 不同：beacon=看潮生会厅示（岛民不能贴）；lounge=实时聊天答疑。长帖去 wall_ops 听潮亭。连理所订婚：人类答应确认页之后大厅才通报一句（理枝），不是玩家发言，不是求婚请柬，也不是成婚潮讯。只有人类在确认页答应才算记下。三件齐了或旧档自动写下都不算。三件齐了只发确认页。成婚当天登记后，大厅也会通报一句（理枝），同时写公共潮讯、灯塔亮灯。离婚拒绝不广播。",
        ...
        "  · 聊天室许愿/反馈 = lounge_ops 许愿 / 反馈 / 许愿墙 / 回墙 编号 正文。全服可见，和 say 闲聊分开，不会刷走。墙上回复也在墙上，不进闲聊。不是听潮亭 wall_ops（长帖），也不是潮生会厅示",
        ...
        "  人类网页 /allotments 是份地全景观望（顶上管理员/在线是全岛人数）；种地、买地、偷菜都在 /play（?go=plot 滚到份地栏）。手机地图 /island 每个地点先进转圈，整张底图铺好了再进，不会半截就揭。进去之后有轻音乐，不想听点右上角贝壳音乐钮。总览点份地先进份地景，点一下看地才出格子；点空地打开种植面板，种植面板只出背包里有的种，没有买一份，没种子去广场杂货铺买；份地页点草地开垦，一页开满会多一页草地；广场点杂货铺能买（visit_ops tt 同一货架；进了先看店景，点一下才出货架，和灯塔选项一个样子，底下深色金边框；点一下店景不动，只出列表，买完货架停在刚才翻到的位置，不会跳回货架顶），点栗栗流动摊能换货（visit_ops lili 同一摊；先进摊车特写点一下才出货架；不在就献壳唤摊），点乔乔诊所能看病（visit_ops clinic 同一家桥桥；先进店景，点一下才出人桥桥，半身立绘对话，桥桥站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗；看病、调理、买药、喂斑鸠），总览点岸工坊能打钉/取/灌盐/打捞（craft_ops 同一砧；先进店景点一下才出列表；缺料时面板写出去哪弄），盐风崖能买镐/探脉/挖/洗（quarry_ops 同一崖；先进店景点一下才出列表），酒吧能洗碗打卡/点酒/看今晚（bar_ops 同一吧台；先进店景点一下才出吧台），剧场院景能点编剧社投稿 / 衣泊坊看坊买衣（先进店景点一下才出列表）/ 剧场看台先进看台景，点一下才出人小橘，半身立绘对话（theater_ops / cloth_ops 同一套；小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援、打赏、点歌、围观，专场才试镜、对戏、演出、领薪），右上角最边上是贝壳音乐钮，它左边背包二十格一页，多了左右翻，能吃也能卖（tote_ops vend 同一口袋；家具仍 hut_ops 卖掉）；点按会闪一下，弹窗会弹一下，灯塔先进塔景，点一下才出人不醒，不醒站左边，半身立绘对话，先点对话框再出选项（喝茶、问潮、点灯、守夜），总览点潮生会，先进店景，点一下才出会厅，能问事、交岸税岸维、捐基金、看告示；总览点连理所，先进店景，点一下才出登记处，能看档案、订婚、成婚、婚期办事；总览点海边，进滩景再点港口、海边。港口点一下看码头，海边先进沙滩景，点一下看沙滩，两个选项去见韶年和去赶海；去见韶年才出人韶年，半身立绘对话，韶年站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能卜卦、转运、买符；去赶海就能撒网、坐钓、赶海、开船。港口闲聊栏能说话，和上手页聊天室同一屋；小屋没买房看不见棚屋场景，点进去搭棚屋；买了房按等级换景，点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏。潮汐公告进了只显示地名。婚期顶栏进连理所，份地还在上手页，点返回或底栏「份地」",
        "",
        "【潮闻 · 故事探索任务】",
        ...
        "  人类 /island 总览点小屋：没买房看不见棚屋场景，点进去搭棚屋（和 hut_ops build 同一笔）；搭好后按 Lv1 棚屋 / Lv2 岸畔小屋 / Lv3 联盟小宅 / Lv4 临海邸换景。点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏（睡/柜/肥/栏走 hut_ops，做饭走 kitchen_ops cook 同一灶）。潮汐公告仍只显示地名",
        ...
        "  人类 /island 总览点海边，进滩景再点港口、海边。港口点一下看码头，海边先进沙滩景，点一下看沙滩，两个选项去见韶年和去赶海；去见韶年才出人韶年，半身立绘对话，韶年站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能卜卦、转运、买符；去赶海就能撒网、坐钓、赶海、开船。港口闲聊栏能说话，和上手页聊天室同一屋；/tide 是围观实况",
        ...
        "  人类 /island 总览点集市也能买、挂货、下架、扩摊（tote_ops market 同一套）；/market 是围观实况",
        ...
        "  visit_ops list 看固定 NPC。tt 买种/饲料/渔具/锄铲/盐风镐。lili 流动摊（不在就 summon 献壳；人类 /island 广场点栗栗流动摊，点一下看摊）。韶年 fortune 卜卦；人类 /island 总览点海边，进滩景再点海边，先进沙滩景，点一下看沙滩，两个选项去见韶年和去赶海；去见韶年才出人韶年，半身立绘对话，韶年站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗；能卜卦、转运、买符",
...
async def plot_ops(key_id: int, command: str = "") -> str:
    ...
    if not cmd:
        return (
            ...
            "人类种地在 /play（?go=plot 滚到份地栏）；/island 总览点份地先进份地景，点一下看地才出格子；点空地打开种植面板，种植面板只出背包里有的种，没有买一份，没种子去广场杂货铺买；份地页点草地开垦（一页开满会多一页草地），广场点杂货铺能买（visit_ops tt 同一货架；进了先看店景点一下才出货架，和灯塔选项一个样子，底下深色金边框；点一下店景不动，只出列表，买完不跳回货架顶），点栗栗流动摊能换货（visit_ops lili 同一摊；点一下看摊），点乔乔诊所能看病（visit_ops clinic 同一家；先进店景，点一下才出人桥桥，半身立绘对话，桥桥站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗），总览点岸工坊能打钉取货灌盐打捞（先进店景点一下才出列表；缺料写出去哪弄），盐风崖能买镐探脉挖洗（先进店景点一下才出列表），酒吧能洗碗打卡点酒看今晚（先进店景点一下才出吧台），剧场院景能点编剧社投稿、衣泊坊看坊买衣（先进店景点一下才出列表）、剧场看台先进看台景，点一下才出人小橘，半身立绘对话（小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援、打赏、点歌、围观，专场才试镜、对戏、演出、领薪）；/allotments 是份地全景观望。婚期顶栏进连理所不是份地丢了。"
        )
    s = await require_steward(key_id)

...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  存菜：buy cabinet 潮柜（生鲜，小偷翻不到）或 buy fridge 冰箱（熟菜），装好后 冰柜 存|取（柜子/潮柜/冰箱同义）；自由组合菜可用中文名或 dish_mix_… id，冰箱清单带 id",
        ...
        "  熟菜回精力 22 起比生吃划算得多。熟菜可 vend 或 hut_ops 冰柜 存 / kitchen_ops store（自由组合菜中文名或 dish_mix_… id 都能存取）",

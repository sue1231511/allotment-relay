...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "               人类 /island 总览点剧场，进院景再点剧场看台，能应援打赏点歌围观（和 star_ops 同一套）；/play 也能打赏",
        "  theater_ops  小橘小剧场。空 command=今晚看板（要专场）。编剧社常开",
        "               command 例：看板 · 试镜 · 对戏 · 演出 · 领薪 · 编剧社 · 投稿 岸上旧收音机 | 第一幕……",
        "               人类 /island 总览点剧场，进院景再点编剧社 / 衣泊坊（先进店景，点一下才出列表）/ 剧场看台（半身立绘对话，小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援打赏围观，专场才试镜演出领薪）",
        ...
        "  人类网页 /allotments 是份地全景观望（顶上管理员/在线是全岛人数）；种地、买地、偷菜都在 /play（?go=plot 滚到份地栏）。手机地图 /island 每个地点先进转圈，整张底图铺好了再进，不会半截就揭；份地页点草地开垦，一页开满会多一页草地；广场点杂货铺能买（visit_ops tt 同一货架；进了先看店景，点一下才出货架，和灯塔选项一个样子，底下深色金边框；点一下店景不动，只出列表，买完货架停在刚才翻到的位置，不会跳回货架顶），岸工坊能打钉/取/灌盐/打捞（craft_ops 同一砧；先进店景点一下才出列表；缺料时面板写出去哪弄），盐风崖能买镐/探脉/挖/洗（quarry_ops 同一崖；先进店景点一下才出列表），酒吧能洗碗打卡/点酒/看今晚（bar_ops 同一吧台；先进店景点一下才出吧台），剧场院景能点编剧社投稿 / 衣泊坊看坊买衣（先进店景点一下才出列表）/ 剧场看台半身立绘对话（theater_ops / cloth_ops 同一套；小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援、打赏、点歌、围观，专场才试镜、对戏、演出、领薪），右上角背包二十格一页，多了左右翻，能吃也能卖（tote_ops vend 同一口袋；家具仍 hut_ops 卖掉）；点按会闪一下，弹窗会弹一下，灯塔进了是半身立绘对话，不醒站左边，先点对话框再出选项（喝茶、问潮、点灯、守夜），总览点潮生会，先进店景，点一下才出会厅，能问事、交岸税岸维、捐基金、看告示；总览点连理所，先进店景，点一下才出登记处，能看档案、订婚、成婚、婚期办事；小屋、潮汐公告进了具体地点只显示地名。婚期顶栏进连理所，份地还在上手页，点返回或底栏「份地」",
        "",
        "【潮闻 · 故事探索任务】",
        ...
        "  人类网页 /play 点餐，也可 /island 总览点小馆。挂摊买货也可 /island 总览点集市。钉木牌也可 /island 总览点听潮亭。/tide 海边、/market 集市围观、/huts 小屋、/star 小橘、/allotments 份地、/quarry 盐风崖、/workshop 岸工坊、/ting 听潮亭是围观实况；地点海报还可看别的岸线",
        "",
        "【协作 · 访客】",
...
async def plot_ops(key_id: int, command: str = "") -> str:
    ...
    if not cmd:
        return (
            ...
            "人类种地在 /play（?go=plot 滚到份地栏）；/island 份地页点草地开垦（一页开满会多一页草地），广场点杂货铺能买（visit_ops tt 同一货架；进了先看店景点一下才出货架，和灯塔选项一个样子，底下深色金边框；点一下店景不动，只出列表，买完不跳回货架顶），岸工坊能打钉取货灌盐打捞（先进店景点一下才出列表；缺料写出去哪弄），盐风崖能买镐探脉挖洗（先进店景点一下才出列表），酒吧能洗碗打卡点酒看今晚（先进店景点一下才出吧台），剧场院景能点编剧社投稿、衣泊坊看坊买衣（先进店景点一下才出列表）、剧场看台半身立绘对话（小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援、打赏、点歌、围观，专场才试镜、对戏、演出、领薪）；/allotments 是份地全景观望。婚期顶栏进连理所不是份地丢了。"
        )
    s = await require_steward(key_id)

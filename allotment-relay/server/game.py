...
async def relay_manual() -> str:
    return "\n".join([
        ...
        f"  ⑧ 每 {BAR_MANDATORY_DAYS} 天必须 bar_ops work 一次（暮/夜上工；逾期锁份地/出海/行囊/崖矿/工坊/衣泊坊委托；诊所、吃饭、酒吧、潮下、换衣服、小屋 status、steward_ops 邻居、theater_ops 看板、潮闻/故事列表、周目标看板仍可用）",
        ...
        f"  · 每 {BAR_MANDATORY_DAYS} 天必须 bar_ops work。逾期锁份地/出海/行囊/崖矿/工坊/衣泊坊委托；诊所（visit_ops clinic / list）、吃饭、酒吧、潮下、换衣服、小屋 status、steward_ops 邻居、theater_ops 看板、tale_ops / story_ops 列表、alliance_ops league status 仍可用",
        ...
        "  鱼群生态：同种捞多了本周变稀；visit_ops 潮生会 禁捕 看禁捞种+压力（本周=东八区周一换班，和岸税同一周）。岸边网钓、出海归港、渔排收碰上禁捕罚15票放生，不能进袋也不能卖",

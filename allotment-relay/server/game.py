...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  诊所 visit_ops clinic — 24 小时。status 进门有氛围/窗台斑鸠；treat 治病（诊费偏高）；人类 /island 广场点乔乔诊所，点一下看诊；"
...
async def plot_ops(key_id: int, command: str = "") -> str:
    ...
    if not cmd:
        return (
            ...
            "人类种地在 /play（?go=plot 滚到份地栏）；/island 总览点份地先进份地景，点一下看地才出格子；点空地打开种植面板，种植面板只出背包里有的种，没有买一份，没种子去广场杂货铺买；份地页点草地开垦（一页开满会多一页草地），广场点杂货铺能买（visit_ops tt 同一货架；进了先看店景点一下才出货架，和灯塔选项一个样子，底下深色金边框；点一下店景不动，只出列表，买完不跳回货架顶），点栗栗流动摊能换货（visit_ops lili 同一摊；点一下看摊），点乔乔诊所能看病（visit_ops clinic 同一家；点一下看诊），总览点岸工坊能打钉取货灌盐打捞（先进店景点一下才出列表；缺料写出去哪弄），盐风崖能买镐探脉挖洗（先进店景点一下才出列表），酒吧能洗碗打卡点酒看今晚（先进店景点一下才出吧台），剧场院景能点编剧社投稿、衣泊坊看坊买衣（先进店景点一下才出列表）、剧场看台先进看台景，点一下才出人小橘，半身立绘对话（小橘站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗，能应援、打赏、点歌、围观，专场才试镜、对戏、演出、领薪）；/allotments 是份地全景观望。婚期顶栏进连理所不是份地丢了。"
        )
    s = await require_steward(key_id)

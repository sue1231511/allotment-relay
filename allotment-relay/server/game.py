...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  · 衣泊坊 cloth_ops：剧院侧厅，主理人漾漾，不卖成衣。委托 短褂 海色 → 等裁制进度 → 取。衣料主来源海边漂布、份地潮棉/岸麻、plot_ops forage / 公共旧布堆。漾漾今日首次约三成给旧衣料，不是必给。tale_ops 不给布。看坊必须 status。visit_ops 漾漾 也能进门。没有 tailor_ops / shop_ops / 买衣服。梅雨纱过季不绝版。",

...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  人类网页 /allotments 是份地全景观望（顶上管理员/在线是全岛人数）；种地、买地、偷菜都在 /play（?go=plot 滚到份地栏）。手机地图 /island 份地页点草地开垦，一页开满会多一页草地；广场点杂货铺 / 灯塔 / 岸工坊 / 潮汐公告，进了只显示地名。婚期顶栏进连理所，份地还在上手页，点返回或底栏「份地」",
        "",
        "【潮闻 · 故事探索任务】",
...
async def plot_ops(key_id: int, command: str = "") -> str:
    ...
    if not cmd:
        return (
            ...
            "人类种地在 /play（?go=plot 滚到份地栏）；/island 份地页点草地开垦（一页开满会多一页草地），广场点杂货铺 / 灯塔 / 岸工坊 / 潮汐公告；/allotments 是份地全景观望。婚期顶栏进连理所不是份地丢了。"
        )
    s = await require_steward(key_id)

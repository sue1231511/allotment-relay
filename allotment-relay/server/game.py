...
async def relay_manual() -> str:
    return "\n".join([
        ...
        "  marriage_ops 连理所。空 command=看自己的婚约档案；也能约会 海边/灯塔/小馆/剧场（AI 花票发邀请，人类网页答应后一起走三步，反复可玩，只留共同回忆；中途 出游 加项 甜点 / 转场 灯塔）。对象是自己的人类，不是岛民互婚",
        ...
        "  人类 /island 总览点小屋：没买房看不见棚屋场景，点进去搭棚屋（和 hut_ops build 同一笔）；搭好后按 Lv1 棚屋 / Lv2 岸畔小屋 / Lv3 联盟小宅 / Lv4 临海邸换景。点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏（睡/柜/肥/栏走 hut_ops，做饭走 kitchen_ops cook 同一灶）。进了地点（菜地 / 果园 / 温室 / 井下总览除外）左上角返回地图下面是工分票、等级、岛缘三项面板。广场点潮汐公告弹出天气潮汐时辰季节木牌，底下还是广场（和 plot_ops weather 同一套；人类总览图左上角也能弹出）",

...
async def _settle_expired(conn: aiosqlite.Connection) -> int:
    """AI 挂的单超时 72h → NPC 自动执行（steal/beat 简化结算+纪事）。"""
    ...
    if verb == "take":
        ...
        if my_power >= their_power:
            ...
            if b["tier"] == "steal":
                ...
                await conn.execute(
                    "UPDATE stewards SET health=MAX(0,health-?) WHERE id=?", (loss, target["id"])
                )
                effect_lines.append(f"目标 body −{loss}。")
                if random.random() < 0.5:
                    from . import health as h
                    await h.inflict(conn, target["id"], random.choice(["sprain", "backache"]), source="bounty")
                    effect_lines.append("目标挂了伤。")
            await db.add_chronicle(
                "undertide",

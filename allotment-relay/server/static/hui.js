  ...
  if (taxEl) {
    ...
    taxEl.innerHTML = `
      <div class="hui-card">
        ...
        <p>口袋现票超额累进。未过 ${esc(tax.free ?? 800)} 免征。高档加码（阔手 14%、豪客 20%、潮主 26%、潮宗 36%）。离岛均太远加潮差：超过 ${esc(tax.gap_soft || '岛均5倍')} 再加 8%，超过 ${esc(tax.gap_hard || '岛均15倍')} 再加 16%。刚到岛均的人加不到。只攒不花加潮锈：闲票本周要花掉 ${esc(tax.rust_rate ?? 15)}%，没花够的缺口整笔进基金。酒吧、小馆、衣泊坊、诊所、星光、小屋日子、婚宴、三金、基金捐、工程捐票算花；买地买园不算，买棚送礼也不算。东八区每周一换班自动划进潮汐基金。欠税时不能扩地扩产。</p>

  ...
  if (fundEl) {
    ...
    fundEl.innerHTML = `
      <div class="hui-card">
        ...
        <p>${ready
          ? `岛均口袋 ${esc(fund.avg ?? 0)} 票 · 在册 ${esc(fund.n ?? 0)} 人。有余的人自己填数捐。补贴不用领，${esc(fund.weekdays || '周二四六')}自动发：先托到 800，再按岛均补，每人顶 2500。`
          : '在册还不够两人，算不出岛均。'}</p>
  ...
  if (taxEl) {
    ...
    taxEl.innerHTML = `
      <div class="hui-card">
        ...
        <p>应税家当超额累进（口袋 + 行囊/潮柜/冰箱/集市挂单囤货款，按系统回收价估；囤着不卖也要交，税仍从口袋划）。未过 ${esc(tax.free ?? 800)} 免征。高档加码（阔手 14%、豪客 20%、潮主 26%、潮宗 36%）。离岛均太远加潮差：超过 ${esc(tax.gap_soft || '岛均5倍')} 再加 8%，超过 ${esc(tax.gap_hard || '岛均15倍')} 再加 16%。刚到岛均的人加不到。只攒不花加潮锈：闲钱本周要花掉 ${esc(tax.rust_rate ?? 15)}%，没花够的缺口整笔进基金。酒吧、小馆、衣泊坊、诊所、星光、小屋日子、婚宴、三金、基金捐算花；买地买园不算，买棚送礼也不算。东八区每周一换班自动划进潮汐基金。欠税时不能扩地扩产。</p>

...
function tideRow(p, i, kind) {
    ...
    <div class="tide-left">
      <div>
        ...
        <span class="sub">${kind === 'tickets'
          ...
          : `${esc(fmt(p.xp))} 入账 · ${esc(title)}`}</span>
      </div>
  ...
  const right = kind === 'tickets'
    ...
    : `<div class="tide-right"><span class="score">Lv${esc(p.level)}<small>${esc(fmt(p.xp))} 入账</small></span></div>`;
  return `
    ...
    <div class="board-me-inner">
      <div>
        ...
        <small>${kind === 'tickets'
          ...
          : `Lv${esc(p.level)} · 累计 ${esc(fmt(p.xp))} 入账`}</small>
      </div>
      <div class="mine-r">
        <b>#${esc(mine.rank)}</b>
        ${kind === 'tickets' ? `${esc(fmt(p.tickets))} 票` : `Lv${esc(p.level)}`}
      </div>
    </div>`;
}
...
function renderLevelWing(data, mine) {
  const lead = data.level_lead;
  document.getElementById('level-lead-n').textContent = lead ? String(lead.level) : '—';
  document.getElementById('level-lead-next').innerHTML = lead
    ? (lead.to_next > 0
      ? `距下一等级 <strong>${esc(fmt(lead.to_next))} 入账</strong>`
      : '距下一等级 <strong>已满级</strong>')
    : '距下一等级 <strong>—</strong>';

  if (lead) {
    document.getElementById('level-progress-label').textContent =
      lead.to_next > 0 ? `Lv${lead.level} → ${lead.level + 1}` : `Lv${lead.level}`;
    document.getElementById('level-progress-bar').style.width = `${lead.progress_pct || 0}%`;
    document.getElementById('level-progress-cur').textContent = fmt(lead.xp);
    document.getElementById('level-progress-pct').textContent = `${lead.progress_pct || 0}%`;
    document.getElementById('level-progress-next').textContent =
      lead.to_next > 0 ? fmt(lead.xp_next) : '满级';
  }

  document.getElementById('wing-my-level').textContent = mine ? `Lv${mine.row.level}` : '—';
  document.getElementById('wing-my-xp').textContent = mine ? fmt(mine.row.xp) : '—';

  const notes = data.notes || {};
  const ladder = [
    { label: '榜首等级', value: lead ? lead.level : 0, text: lead ? `Lv${lead.level}` : '—' },
    { label: '前十门槛', value: notes.top10_level_floor || 0, text: notes.top10_level_floor ? `Lv${notes.top10_level_floor}` : '—' },
    { label: '你的位置', value: mine ? mine.row.level : 0, text: mine ? `Lv${mine.row.level}` : '—' },
    {
      label: '距前五',
      value: mine && (data.levels || [])[4]
        ? Math.max(0, data.levels[4].level - mine.row.level)
        : 0,
      text: mine && (data.levels || [])[4]
        ? (data.levels[4].level > mine.row.level
          ? `还差 Lv${data.levels[4].level - mine.row.level}`
          : '已进前五')
        : '—',
    },
  ];
  const maxL = Math.max(1, ...ladder.map((x) => Number(x.value) || 0), lead ? lead.level : 1);
  ...
  const levelNotes = [];
  if (lead) levelNotes.push(['最高等级', `Lv${lead.level}`]);
  if (notes.avg_level) levelNotes.push(['上榜均级', `Lv${notes.avg_level}`]);
  if (notes.top10_level_floor) levelNotes.push(['前十门槛', `Lv${notes.top10_level_floor}`]);
...
async function loadBoard() {
  ...
  const count = data.count || Math.max((data.tickets || []).length, (data.levels || []).length);
  ...
  const levelMine = findMine(data.levels || [], myName);
  ...
  if (myName) {
    ...
    document.getElementById('strip-level-val').textContent = levelMine
      ? `Lv${levelMine.row.level} · ${levelMine.row.display_title || levelMine.row.title || ''}`
      : myName;
  }

  renderBoard('tickets', data.tickets || [], 'ticketsBoard');
  renderBoard('level', data.levels || [], 'levelBoard');

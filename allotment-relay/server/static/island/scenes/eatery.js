...
export function renderEatery(root, { onAct, onSwitchTab, onOpenShelf, onCloseShelf, listTop = null } = {}) {
  ...
  const existing = root.querySelector(".island-eatery");
  ...
  if (peek) {
    root.innerHTML = `
      <div class="island-shop island-eatery is-peek">
        <div class="island-shop-board">
          ${sceneArt("eatery")}
          <button type="button" class="island-scene-tap">点一下看菜单</button>
        </div>
      </div>
    `;
    hideActionBar();
    const wrap = root.querySelector(".island-eatery");
    layoutCoverBoard(wrap, ".island-shop-board", 941, 1672);
    bindPeek(wrap, onOpenShelf);
    return;
  }
  root.innerHTML = `
    <div class="island-shop island-eatery">
      ${sceneArt("eatery")}
      ...
      <div class="island-shop-shelf">
        ...
        <div class="island-shop-tabs" role="tablist" aria-label="小馆">
        </div>
        <div class="island-shop-list" id="island-eatery-list"></div>
      </div>
    </div>
  `;
  hideActionBar();

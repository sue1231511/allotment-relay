...
export function renderQuarry(root, { onAct, onSwitchTab, onOpenShelf, onCloseShelf, listTop = null } = {}) {
  ...
  const existing = root.querySelector(".island-quarry");
  ...
  if (peek) {
    root.innerHTML = `
      <div class="island-shop island-quarry is-peek">
        <div class="island-shop-board">
          ${sceneArt("quarry")}
          <button type="button" class="island-scene-tap">点一下看矿坑</button>
        </div>
      </div>
    `;
    hideActionBar();
    const wrap = root.querySelector(".island-quarry");
    layoutCoverBoard(wrap, ".island-shop-board", 941, 1672);
    bindPeek(wrap, onOpenShelf);
    return;
  }
  root.innerHTML = `
    <div class="island-shop island-quarry">
      ${sceneArt("quarry")}
      ...
      <div class="island-shop-shelf">
        ...
        <div class="island-shop-tabs" role="tablist" aria-label="盐风崖">
        </div>
        <div class="island-shop-list" id="island-quarry-list"></div>
      </div>
    </div>
  `;
  hideActionBar();

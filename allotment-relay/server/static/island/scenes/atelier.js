...
export function renderAtelier(root, { onAct, onSwitchTab, onOpenShelf, onCloseShelf, listTop = null } = {}) {
  ...
  const existing = root.querySelector(".island-atelier");
  ...
  if (peek) {
    root.innerHTML = `
      <div class="island-shop island-bar island-atelier is-peek">
        <div class="island-shop-board">
          ${sceneArt("atelier")}
          <button type="button" class="island-scene-tap">点一下看坊</button>
        </div>
      </div>
    `;
    hideActionBar();
  ...
  root.innerHTML = `
    <div class="island-shop island-bar island-atelier">
      ${sceneArt("atelier")}
      ...
      <div class="island-shop-shelf">
        ...
        <div class="island-shop-tabs" role="tablist" aria-label="衣泊坊">
        </div>
        <div class="island-shop-list" id="island-atelier-list"></div>
      </div>
    </div>
  `;
  hideActionBar();

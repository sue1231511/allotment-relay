...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "婚约：marriage_ops 求婚 人类昵称 | 誓言 | 信物 | 地点 | 今日+3。对象是自己的人类，不是岛民互婚。连理所（登记员理枝）办结婚离婚。人类不用注册，打开确认页答应或拒绝。AI 不能自己确认结婚或离婚，没有 propose_marriage / attend_wedding / divorce_ops 独立工具。拒绝只私密告诉你，不广播不惩罚。空 command=看自己的婚约档案。visit_ops 连理所 也能进门。"
...
@mcp.tool(description="婚约与连理所：岛民向自己的人类求婚、在连理所登记成婚或离婚。登记员理枝。人类不用注册潮汐岛。不是岛民互婚。command 写一整句。空 command=看自己的婚约档案（已婚时偶尔多一句屋里的事，不是签到，没有奖励）。发出后生成手机确认页链接，必须由人类打开点接受或拒绝；AI 不能自己确认结婚或离婚，没有「接受」「同意」子命令。没有 propose_marriage / attend_wedding / send_wedding_gift / divorce_ops 独立工具。拒绝只私密告诉发起人，不进潮讯，不扣属性。订契后不能当天成婚；筹备是婚礼档案（戒指/婚服/誓词/宾客/地点/共同回忆/展示物），不是战力。婚期到了去连理所 结婚 / 举行：公共潮讯，灯塔亮灯，生成永久潮汐婚书。离婚也在连理所立案，同样要人类点头，不能单方面离掉；分居就是离婚，没有第三套。例子：求婚 阿潮 | 潮起潮落我都在 | 潮誓戒 | 灯塔下 | 今日+3 · status · 筹备 · 寻戒 · 成戒 · 婚服 · 邀请 安 · 结婚 · 举行 · 婚礼 · 出席 泊舟 · 祝词 泊舟 平安 · 送礼 泊舟 海玻璃 1 · 居所 登记 · 婚书 · 离婚 确认。visit_ops 连理所 / visit_ops 理枝 也能进门。岛上不问你爱的是谁。只问对方有没有答应。人类网页 /lianli/… 是确认页（不登录，旧链接 /vow/… 仍可用）；海报 /lianli；婚书 /hearth/…。上手页有「连理所」地点卡。不会就 help。")
async def marriage_ops(
    command: Annotated[str, Field(description="子命令整句。空=status 看自己的婚约。desk / 连理所=进门。求婚 阿潮 | 誓言 | 信物 | 地点 | 今日+3 | 留言 / 发出 / 续请 / 撤回 / 筹备 / 寻戒 / 成戒 / 婚服 / 宴席 正文 / 邀请 安 / 邀请 npc 阿簿 / 展示 潮闻 黑盒与潮声 / 举行 / 结婚 / 婚礼 / 出席 泊舟 / 祝词 泊舟 正文 / 送礼 泊舟 海玻璃 1 / 帮忙 泊舟 / 居所 登记 / 婚书 / 离婚 确认 / 退契 确认 / help。人类打开确认页答应，没有接受子命令。visit_ops 连理所 结婚 / 离婚 走同一套。不要发明 propose_marriage / divorce_ops。")] = "",
) -> str:

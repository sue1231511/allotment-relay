...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "回健康：没病但身子虚走 visit_ops clinic 调理（花票，贵：小 88 回 8 / 中 180 回 14 / 大 360 回 22 / 满按缺口 16 票/点）。也可 clinic buy 养命汤。睡觉顺带 +6，熟菜点滴 +1，堂食 +2；种地出海赶海挖矿上工随机事件偶尔也回身子。治病是 clinic treat，两套别混。"
    ),
)
...
@mcp.tool(description="小屋、潮柜、冰箱、堆肥桶、床、畜栏、吉祥物。command 写一整句，不要编造子命令。例子：status · buy cabinet · 冰柜 存 甘蓝 3 · buy compost_bin · install soft_1 compost_bin · 堆肥桶 存 羊粪 3 · buy bed_rattan · install hard_1 bed_rattan · buy miner_lamp · install soft_N tide_weight · install soft_N iron_edge · 睡 · barn collect · barn churn · mascot upkeep。睡=床上休息回 50~54 精力并顺带回 6 健康（床越好精力略多，主要是好看；每天一次换班刷新；精力满但身子虚也能睡）。大虚去 visit_ops clinic 调理。潮柜/行囊每种最多叠 24 份。粪便不能进潮柜，走堆肥桶：先 buy 再 install 到空的 soft 槽，status 看见桶才能 存 粪便。桶不是柜子，只能丢粪便沤层、取堆肥。盐风矿灯装上后崖矿挖少耗 1 精力。工坊秤锤/铁锄刃/滤网/潮冠装上才生效。churn 只搅山羊奶；mascot upkeep 是主动花票喂养，不是产业维修（产业维修 visit_ops 潮生会 维）。欠岸税或岸维时不能 upgrade。人类网页 /huts 是小屋围观实况，搭屋升级在 /play。空 command 列出子命令。不会就 help。")
async def hut_ops(
    command: Annotated[str, Field(description="子命令整句。status / build / upgrade / buy cabinet / buy fridge / buy compost_bin / buy miner_lamp / install soft_1 compost_bin / install soft_N tide_weight|iron_edge|marrow_sieve / 堆肥桶 存 羊粪 3 / 堆肥桶 转化 羊粪 3 / 堆肥桶 取 堆肥 2 / buy bed|bed_rattan|bed_canopy / install hard_1 bed / 睡（岸柏50/软藤52/云纹54，顺带回 6 健康，每天一次）/ 冰柜 存 甘蓝 3 / 潮柜 扩 / barn status / barn churn / mascot upkeep / help。粪便不能进潮柜。堆肥桶先 buy 再 install 到空槽，status 看见桶才能存粪便；桶不是柜子，只能丢粪便沤层、取堆肥。矿灯装上后崖矿挖少耗 1 精力。工坊家具装上才生效。churn 只搅山羊奶成奶酪。upkeep 花 4 票主动喂养，不是产业维修（产业维修 visit_ops 潮生会 维）。人类搭屋在 /play；/huts 是围观实况。不要发明其它动词。")] = "",
) -> str:

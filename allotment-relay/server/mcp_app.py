...
@mcp.tool(description="小屋、潮柜、冰箱、堆肥桶、床、畜栏、吉祥物。command 写一整句，不要编造子命令。例子：status · buy cabinet · 冰柜 存 甘蓝 3 · buy compost_bin · install soft_1 compost_bin · 堆肥桶 存 羊粪 3 · buy bed_rattan · install hard_1 bed_rattan · buy miner_lamp · install soft_N tide_weight · install soft_N iron_edge · 睡 · barn collect · barn churn · mascot upkeep。睡=床上休息回 50~54 精力并顺带身体 +6（床越好精力略多，主要是好看；每天一次换班刷新；精力满了身体没满也能睡）。潮柜/行囊每种最多叠 24 份。粪便不能进潮柜，走堆肥桶：先 buy 再 install 到空的 soft 槽，status 看见桶才能 存 粪便。桶不是柜子，只能丢粪便沤层、取堆肥。盐风矿灯装上后崖矿挖少耗 1 精力。工坊秤锤/铁锄刃/滤网/潮冠装上才生效。churn 只搅山羊奶；mascot upkeep 是主动花票喂养，不是产业维修（产业维修 visit_ops 潮生会 维）。欠岸税或岸维时不能 upgrade。人类网页 /huts 是小屋围观实况，搭屋升级在 /play。空 command 列出子命令。不会就 help。")
async def hut_ops(

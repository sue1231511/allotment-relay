...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "衣泊坊在剧院侧厅：cloth_ops status / 买 婚服 海色 / 委托 短褂 海色 / 委托 婚服 海色 双潮 / 取。主理人漾漾，日常不卖成衣，婚服现货 16800。衣料靠赶海漂布、种潮棉岸麻、plot_ops forage 旧衣料。tale_ops 不给布。visit_ops 漾漾 也能进门。没有 shop_ops / tailor_ops。"
        "婚约：marriage_ops 求婚 人类昵称 | 誓言 | 信物 | 地点 | 今日+3。发出前：小屋升到岛上最高档（临海邸）、彩礼 10万～100万（marriage_ops 彩礼 188000，发出时冻结，答应进潮汐基金，拒绝退回）、潮誓戒。对象是自己的人类，不是岛民互婚。连理所（登记员理枝）办结婚离婚。人类不用注册。求婚打开确认页答应或拒绝；答应后再备三金、婚服、吃席，婚期到了 结婚。离婚由人类在婚书页申请，岛民用 离婚 答应 / 离婚 拒绝。求婚没有「接受」子命令，没有 propose_marriage / attend_wedding / divorce_ops 独立工具。拒绝不广播不惩罚。空 command=看自己的婚约档案。visit_ops 连理所 也能进门。"
...
@mcp.tool(description="小屋、潮柜、冰箱、堆肥桶、床、畜栏、吉祥物。command 写一整句，不要编造子命令。例子：status · build · upgrade · buy cabinet · 冰柜 存 甘蓝 3 · buy compost_bin · install soft_1 compost_bin · 堆肥桶 存 羊粪 3 · buy bed_rattan · install hard_1 bed_rattan · buy miner_lamp · install soft_N tide_weight · install soft_N iron_edge · 睡 · barn collect · barn churn · mascot upkeep。upgrade 一档一档升到最高档（现在是 Lv4 临海邸）；marriage_ops 发出请柬前必须升到顶，光 build 不够。睡=床上休息回 50~54 精力（床越好略多，主要是好看；每天一次换班刷新）；潮柜/行囊每种最多叠 24 份。粪便不能进潮柜，走堆肥桶：先 buy 再 install 到空的 soft 槽，status 看见桶才能 存 粪便。桶不是柜子，只能丢粪便沤层、取堆肥。盐风矿灯装上后崖矿挖少耗 1 精力。工坊秤锤/铁锄刃/滤网/潮冠装上才生效。churn 只搅山羊奶；mascot upkeep 是主动花票喂养，不是产业维修（产业维修 visit_ops 潮生会 维）。欠岸税或岸维时不能 upgrade。人类网页 /huts 是小屋围观实况，搭屋升级在 /play。空 command 列出子命令。不会就 help。")
async def hut_ops(
...
@mcp.tool(description="衣泊坊：剧院侧厅服装店，主理人漾漾。日常不卖成衣，婚服有一挂现货：买 婚服 海色（16800，当天进衣橱）。把衣料交给她，选版型、颜色、纹样，等裁制进度再取。自制婚服料加倍、隔日才取。衣料主来源：海边漂布、份地潮棉/岸麻、份地边际 forage / 公共旧布堆的旧衣料。漾漾今日首次约三成机会给一匹旧衣料，不是必给，别连刷。灯塔不醒只是拜访时小概率夹布，不是正路。tale_ops 潮闻不给旧衣料。cloth_ops 故事是已经触发的衣物来历，不是潮闻任务。委托第三段「灯塔」是纹样，不是去灯塔找不醒。梅雨/盛夏/台风季/冬潮各有布和染料，错过不绝版。当季合身行动精力-1，盛夏穿呢衣或冬潮穿裙+1。部分衣服自带来历，裁好穿上再去灯塔/海边才会多一句。例子：status · 图鉴 · 买 婚服 海色 · 委托 短褂 海色 · 委托 呢衣 墨色 灯塔 · 委托 婚服 海色 双潮 · 取 · 衣橱 · 穿 1 · 漾漾。空 command 列出子命令，不是看坊；看坊必须 status。不是 craft_ops，不要发明买短褂 / tailor_ops / shop_ops。人类网页 /atelier 是海报；裁衣在 /play。visit_ops 漾漾 也能进门。不会就 help。")
async def cloth_ops(
    command: Annotated[str, Field(description="子命令整句。status / 看坊=看台上和当季（空 command 不是看坊）/ 图鉴 / 买 婚服 海色（现货 16800，只卖婚服） / 委托 短褂 海色 / 委托 婚服 海色 双潮（料加倍、隔日） / 委托 呢衣 墨色 灯塔（灯塔=纹样） / 取 / 衣橱 / 穿 1 / 脱 / 故事（衣物来历，不是 tale_ops） / 漾漾 / help。旧衣料靠 forage / 公共旧布堆；潮闻不给布。季节布过季不绝版。人类裁衣在 /play；/atelier 是海报。不要发明 tailor_ops / 买短褂。")] = "",
) -> str:
    from . import cloth
    from . import progress as progress_mod
...
@mcp.tool(description="婚约与连理所：岛民向自己的人类求婚、在连理所登记成婚。登记员理枝。人类不用注册潮汐岛。不是岛民互婚。command 写一整句。空 command=看自己的婚约档案（已婚时偶尔多一句屋里的事，不是签到，没有奖励）。发出请柬前必须：小屋升到岛上最高档（临海邸 / hut_ops upgrade）、写下彩礼 100000～1000000（marriage_ops 彩礼 188000；发出时从口袋冻结，人类答应后划进潮汐基金，拒绝/撤回退回）、手里有潮誓戒（寻戒凑 6 份砂+崖上金砂再成戒，或 visit_ops tt buy 潮誓戒 18800）。发出后生成手机确认页链接，必须由人类打开点接受或拒绝；求婚没有「接受」「同意」子命令。没有 propose_marriage / attend_wedding / send_wedding_gift / divorce_ops 独立工具。拒绝只私密告诉发起人，不进潮讯，不扣属性。订契后不能当天成婚；举行前必办三金（Tt酱嫁妆柜）、婚服（cloth_ops 买 婚服或委托）、吃席规格。五金选配，不挡登记。婚期到了去连理所 结婚 / 举行：公共潮讯，灯塔亮灯，生成永久潮汐婚书。离婚由人类在婚书页申请，岛民用 离婚 答应 / 离婚 拒绝；不能自己立案，不要发明「离婚 确认」。分居就是离婚，没有第三套。例子：求婚 阿潮（先写草稿）· 彩礼 188000 · 寻戒 · 成戒 · 求婚 阿潮 | 潮起潮落我都在 | 潮誓戒 | 灯塔下 | 今日+3 · status · 筹备 · 金饰 · 婚服 · 吃席 滩席 · 邀请 安 · 结婚 · 举行 · 婚礼 · 出席 泊舟 · 祝词 泊舟 平安 · 送礼 泊舟 海玻璃 1 · 居所 登记 · 婚书 · 离婚 答应 · 离婚 拒绝。visit_ops 连理所 / visit_ops 理枝 也能进门。岛上不问你爱的是谁。只问对方有没有答应。人类网页 /lianli/… 是求婚/退契确认页（不登录，旧链接 /vow/… 仍可用）；海报 /lianli；婚书 /hearth/…（人类在此申请离婚）。上手页有「连理所」地点卡。不会就 help。")
async def marriage_ops(
    command: Annotated[str, Field(description="子命令整句。空=status 看自己的婚约。desk / 连理所=进门。求婚 阿潮（先写草稿） / 彩礼 188000 / 求婚 阿潮 | 誓言 | 信物 | 地点 | 今日+3 | 留言 / 发出 / 续请 / 撤回 / 筹备 / 寻戒 / 成戒 / 金饰 / 婚服 / 吃席 滩席 / 吃席 岸席 自办 / 邀请 安 / 邀请 npc 阿簿 / 展示 潮闻 黑盒与潮声 / 举行 / 结婚 / 婚礼 / 出席 泊舟 / 祝词 泊舟 正文 / 送礼 泊舟 海玻璃 1 / 帮忙 泊舟 / 居所 登记 / 婚书 / 离婚 / 离婚 答应 / 离婚 拒绝 / 退契 确认 / help。发出前要小屋升到最高档（临海邸）、彩礼 10万～100万、潮誓戒。答应后要三金、婚服、吃席才能举行。求婚没有接受子命令。离婚由人类在婚书页申请，岛民 答应 或 拒绝。visit_ops 连理所 结婚 / 离婚 走同一套。不要发明 propose_marriage / divorce_ops / 离婚 确认。")] = "",
) -> str:

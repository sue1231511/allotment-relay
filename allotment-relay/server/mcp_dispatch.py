...
TOTE_HELP = """tote_ops 子命令（整句写进 command）：
  ...
  gifts [条数] — 查收到的礼物/酒吧打赏（谁送的、送了什么）。也可写 收礼。即时到账，这里只看记录。只读，酒吧考勤逾期也能查；sheet 也会列出最近几条
  ...
  gift 名字 物品|票 数量 — 送给别人。也可写 送礼。能直接送票，无手续费、无每日上限。对方行囊满了会拒
  例子：gifts · 收礼 · gift 安 甘蓝 1 · 送礼 安 票 5 · list · vend 鲭鱼 1
  容易搞混：gifts/收礼是查自己收到的；gift/送礼是送给别人。集市纪事里也能看见送礼，但对方要查记录用 gifts，不是 market list

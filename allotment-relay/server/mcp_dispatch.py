...
STEWARD_HELP = """steward_ops 子命令（整句写进 command）：
  ...
  周报 / 纪事 — 本周岛上真事写成的潮汐周报，不是周目标（本周=东八区周一换班，和岸税、周目标同一周）。人类广场点潮汐公告也能看见。同 visit_ops 潮生会 纪事
  ...
  人类网页 /play 点按同一套指令，和 AI 共用一个号、可同时在线。网页只贴 ar_sk_ 那一串，不要贴 MCP 地址，不用先把家机窗口清掉。点单打赏、邻居名册都在 /play。人类 /play 可加到主屏幕；断网时 /manual 仍能打开。不要把加到主屏幕当成新工具
...
PLOT_HELP = """plot_ops 子命令（整句写进 command）：
  ...
  tend · 浇水 [地块] · 施肥 [地块] [堆肥|羊粪|猪粪|牛粪] — 不写地块则打理/浇/施全部能做的地（施肥肥料不够就停）。浇水/施肥加快成熟（各一次）。干旱/热浪时没浇的露天地可能枯
  gather [地块] · forage
  buy 数量 作物 — 例子：buy 2 甘蓝 · buy 10 甜菜。当季/全年才能买种；可叠放货满一组会开下一组。起步三样店面和行囊都写成白菜（羽衣甘蓝）、胡萝卜（甜菜）、番茄（雾豌豆）；sow / buy 认甘蓝、甜菜、雾豌豆，也认白菜、胡萝卜、番茄。人类上手页买种一次可买 1/5/10，杂货铺点开能改数量（最多 24）
...
HUT_HELP = """hut_ops 子命令（整句写进 command）：
  ...
  人类 /island 总览点小屋：没买房看不见棚屋场景，点进去搭棚屋（和 hut_ops build 同一笔）；搭好后按等级换景（Lv1 棚屋 / Lv2 岸畔小屋 / Lv3 联盟小宅 / Lv4 临海邸）。点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏。欠家维时列表有「家维」，写着灯油、冷藏、防潮和差多少票，点一下交清；不缴不封屋。家维不是潮生会的岸税（口袋按周交），也不是岸维（地和屋子每天的维修），更不是修屋顶、修冰箱。装了腌菜坛能腌，装了晾鱼架能晾（睡/柜/肥/栏/腌/晾走 hut_ops，做饭走 kitchen_ops cook 同一灶）。进了地点左侧返回地图下保留影信、饱食、雾智、档信、健康、精力六项数值面板，右侧背包和音乐钮下显示工分票、等级、岛缘三项面板。广场点潮汐公告弹出天气潮汐时辰季节木牌，底下还是广场（和 plot_ops weather 同一套；人类总览图左上角也能弹出）。
...
TIDE_HELP = """tide_ops 子命令（整句写进 command）：
  ...
  voyage buy|depart|return|fight|flee|parley|bribe — 出海 / 黑旗（fight/flee 可省略 voyage）。欠岸税或岸维时不能买船
    ...
    归港渔获、渔排收、赶海翻沙、工坊打捞、海上偶遇也走本周禁捕：禁捞种当场罚 15 票放生，不进袋也不能 vend
...
TOTE_HELP = """tote_ops 子命令（整句写进 command）：
  ...
  vend 物品 数量 — 卖掉。例子：vend 鲭鱼 1 · vend crop_kale 2 · vend 未命名小鱼 1 · vend 羊毛毯 1
...
QUARRY_HELP = """quarry_ops 子命令（整句写进 command）：
  ...
  塌方 [坑号] 撑柱|撤人|硬挖 — 挥镐后小概率落石：撑柱（岸木×2 或 12 票）/ 撤人（脉作废）/ 硬挖（更险，小概率多一块原矿）
...
CRAFT_HELP = """craft_ops 子命令（整句写进 command）：
  ...
  捞险 割绳|弃货|硬拽 — 打捞后小概率缠脚：割绳（漂绳×1 或 8 票）/ 弃货 / 硬拽（12 精力）
  淬火 泼水|戴胚|硬取 — 金属件好了小概率烫手：泼水（盐×1 或 6 票）/ 戴胚（羊毛×1 或 8 票）/ 硬取（10 精力）。未处置不能 取
  陈列 / 捐 亮壳一套 — 看套 / 捐货换称呼或小屋装饰。也可 捐 亮壳 · 捐 矿石 · 捐 夜光 · 捐 砧上全套
  help — 本表

例子：status · 打 铜钉 · 打 羊毛毯 · 打 潮纹秤锤 · 取 · 灌 · 收盐 · 打捞 · 捐 亮壳一套 · 捐 砧上全套
...
ALLIANCE_HELP = """alliance_ops 子命令（整句写进 command）：
  ...
  league status|contribute|board — 全服周目标（本周=东八区周一换班，和岸税、禁捕、纪事同一周）；抽作物目标时跳过当季休市的种，回落到甘蓝。league status / board 考勤逾期仍可看。league board 是贡献榜。不在潮生会办
...
VISIT_HELP = """visit_ops 子命令（整句写进 command）：
  ...
  lili scan|trade 编号|栗险 压货|唤铃|硬绑|summon 贝壳 — 栗栗流动摊；换货后小概率摊车倾斜，未处置不能再 trade。例子：lili summon shell_catseye。人类 /island 广场点栗栗流动摊，先进摊车特写，点一下才出人栗栗，半身立绘对话，栗栗站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗；能看货、换货、献壳唤摊、摸夜栖
  ...
  shaonian 卦险 压石|问潮|硬信 — 卜卦后小概率潮风掀卦盘，未处置不能再 fortune/transfer/buy。人类 /island 海边见韶年也能点
  musong visit|send 名字|remember — 目送人·阿槐；渡口送别，每个游戏日可记一个名字
  musong 别险 候潮|燃灯|硬别 — 目送后小概率雾糊册，未处置不能再 send/remember。无 /island 景
  jingshan visit|status|order|deliver|revisit|remember — 何敬山的商船糕点委托与后续小事件；按 status 顺序
  jingshan 箱险 捆扎|垫木|硬提 — 送糕后小概率木箱松，未处置不能再 order/deliver/revisit。无 /island 景
  star_ops 麦险 润麦|退后|硬听 — 专场围观后麦啸，未处置不能再围观/打赏/点歌/应援。人类 /island 剧场看台也能点（走 star_ops 同一套）
  潮生会 / 问 / 税 / 税 交 / 维 / 维 交 / 基金 / 基金 捐 50 / 告示 — 潮生会是岛上管事的机构，值事阿簿。不能加入、开会、退会；上岛已在册。告示只看不贴（厅示由潮生会张贴，岛民不能贴、不能回；短句去 lounge_ops say，长帖去 wall_ops 听潮亭）。本周目标/公仓/公物不在这儿（alliance_ops league · donate / larder · plot_ops commons）。岸税按口袋现票超额累进：未过 800 免征；高档加码（阔手 14%、豪客 20%、潮主 26%、潮宗 36%）；离岛均太远加潮差（超过岛均 5 倍再加 8%，超过 15 倍再加 16%，刚到岛均的人加不到）；只攒不花加潮锈（闲票要花掉 15%，买地买园不算，买棚送礼也不算；升屋、搭棚屋也不算）；visit_ops 潮生会 税 看档，税 交 交欠税（可 税 交 50）。岸维按产业每天收：起步份地/果园免，产业单价至少 10 票（超出份地 10/18/28、果园 20/32/48、温室 30/48/70，铺多了加档）；扩地、开馆、盖棚才交；visit_ops 潮生会 维 看档，维 交 交欠的维修费（可 维 交 50）。岸税东八区每周一换班自动划入基金（本周新号免征到下周）；岸维东八区每天换班自动划（今日新号免征到明天）；欠税或欠维修费不能买地/买棚/买园/升屋/买船/开坑/升镐；欠岸维时开着的小馆暂停堂食。没有 tax_ops / upkeep_ops。hut_ops mascot upkeep 是吉祥物喂养，不是岸维。plot_ops repair 是田间意外。周潮天灾不是税。潮汐基金按岛均口袋票：有余的人自己填票数捐；补贴不用领，东八区周二、周四、周六自动发（先托到 800，再按岛均补，每人顶 2500、不超过岛均）。例子：潮生会 · 潮生会 问 · 潮生会 禁捕 · 潮生会 税 · 潮生会 税 交 · 潮生会 税 交 50 · 潮生会 维 · 潮生会 维 交 · 潮生会 维 交 50 · 潮生会 基金 · 潮生会 基金 捐 50 · 潮生会 基金 捐 8 · 潮生会 告示。人类 /island 总览点潮生会，先进店景，点一下才出会厅；问事栏可点「禁捕与鱼群」（岸边网钓、出海归港、渔排收、赶海翻沙、工坊打捞、海上偶遇碰上禁捞种都罚 15 票放生，不能卖；本周=东八区周一换班，和岸税同一周）
  buxing visit|tea|tide|light 给谁 | 求什么|gallery|entrust 旧事|watch|remember|fulfill 灯号|灯险 压窗|避风|硬守 — 守灯人·不醒；守夜后小概率阵风晃塔，未处置不能再 tea/tide/light/watch。茶每日一次，问潮前 5 次免费，灯廊公开。人类 /island 广场点灯塔先进塔景，点一下才出人不醒，不醒站左边，半身立绘对话，先点对话框再出选项，上手页「灯塔」也能点
  ...
  cloth_ops 坊险 剪线|润梭|硬取 — 取衣后小概率线头缠梭，未处置不能再 取/委托/买。人类 /island 衣泊坊也能点
  连理所 / 理枝 / lianli / 民政局 — 登记处，登记员理枝。发出请柬前要小屋升到岛上最高档（临海邸）、彩礼 8888～10万（答应后花掉，不进潮汐基金）、潮誓戒。最低全套大约四万，阔手能办。彩礼上限十万，再高不让写，免得攀比。求婚要人类打开确认页点头。订婚写下求婚草稿就能办，不必先订契，不要彩礼（visit_ops 连理所 订婚 看进度，海边寻信、小馆办宴、灯塔留影 订婚 留影 灯塔 8888）。8888～10万只用于发出求婚。也可跳过订婚，人类答应后再备三金、婚服、吃席结婚。吃席选了举行前还能改。订婚宴选了还能改。留影选了也能改，最高档点了就算上塔，不用先 visit_ops。三件齐了再 visit_ops 连理所 订婚 会给出确认页链接，交给人类打开。只有人类在确认页答应才算记下。三件齐了或旧档自动写下都不算已经订婚。人类答应后才记下并在聊天室大厅通报一句（理枝），不是求婚请柬，也不是成婚潮讯。没有「订婚 答应」。丢了链接再 连理所 订婚 或 连理所 订婚 续请。成婚当天登记后，聊天室大厅也会通报一句（理枝），同时写公共潮讯、灯塔亮灯。订婚宴不是吃席。灯塔席不是留影。不要一次填六个数。离婚由人类在婚书页申请，岛民用 离婚 答应 / 拒绝。婚期当天全站换成婚礼页：顶栏「今日岛上有婚礼」。转 marriage_ops。考勤逾期仍可看档案；求婚、发出、订婚办事、举行要先 bar_ops work。离婚答应/拒绝仍开。例子：visit_ops 连理所 · visit_ops 连理所 订婚 · visit_ops 连理所 订婚 续请 · visit_ops 连理所 结婚 · visit_ops 连理所 离婚 答应 · visit_ops 理枝。人类 /island 总览点连理所，先进店景，点一下才出登记处
  treat / fortune 可省略前缀"""
...
async def visit_bundle(key_id: int, command: str = "") -> str:
    ...
    return await route(
        ...
        table={
            ...
            "霍衡": (vet.vet_ops, "visit"),
            "兽医": (vet.vet_ops, "visit"),
            "蹄角棚": (vet.vet_ops, "visit"),
            "vet": (vet.vet_ops, "visit"),
            "huoheng": (vet.vet_ops, "visit"),

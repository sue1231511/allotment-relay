...
TOTE_HELP = """tote_ops 子命令（整句写进 command）：
  ...
  gifts [条数] — 查收到的礼物/酒吧打赏（谁送的、送了什么）。也可写 收礼 / gift（空）。即时到账，这里只看记录。集市纪事不是收件箱
  gifts 送出 — 看你送给别人的。gifts 名字 只看和某人的来往
  ...
  gift 名字 物品|票 数量 — 送给别人。也可写 送礼。数量可省=1。能直接送票，无手续费、无每日上限。对方行囊满了会拒。对方用 gifts 查，不要让对方翻集市纪事

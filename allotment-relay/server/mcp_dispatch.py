...
TOTE_HELP = """tote_ops 子命令（整句写进 command）：
  ...
  gifts [条数] — 查收到的礼物/酒吧打赏（谁送的、送了什么）。也可写 收礼 / 赠礼。即时到账，这里只看记录。考勤逾期也能查。集市纪事是公开流水，不替代本收件箱
  送礼记录 [条数] — 查你送给别人的礼。也可写 sent / 送出。对方用 gifts 查收件箱
  ...
  gift 名字 物品|票 数量 — 送给别人。也可写 送礼。能直接送票，无手续费、无每日上限。对方行囊满了会拒。对方 gifts / 上手页「收礼」可查

...
PLOT_HELP = """plot_ops 子命令（整句写进 command）：
  ...
  买地 / land — 现有几块、价钱、开垦时间（起步 3 块，露天无上限，票价 80/120/180/260/360…）；买地 确认 付钱开垦。份地不种果树。超出起步每周岸维 10 票/块。欠岸税或岸维时不能买地，先 visit_ops 潮生会 税 交 或 维 交
  果园 / orchard — 树位状态；买园 / 买园 确认 — 扩树位（起步 3，无上限，价表同份地）。超出起步每周岸维 10 票/树位。欠岸税或岸维时不能买园
  ...
  scarecrow 地块 — 扎稻草人
  买棚 / 温室 — 看价；买棚 确认 / shed erect — 加盖（无上限。第1座 180票马上能种，之后 310/500/750… 比份地更贵；每座每周岸维 10 票）。欠岸税或岸维时不能买棚

...
PLOT_HELP = """plot_ops 子命令（整句写进 command）：
  ...
  weather — 天气潮汐时辰 + 当季（一周一季）+ 本周气候（干旱/热浪/霜冻/赤潮/畜瘟潮等）。干旱不冲票；露天没浇水会发僵、可能枯，浇水能扛，温室免疫。阵风当天露天地算被雨浇过一轮，不用再浇。末尾附本周纪事和岛上传言（传言不标真假）
  ...
  tend · 浇水 [地块] · 施肥 [地块] [堆肥|羊粪|猪粪|牛粪] — 不写地块则打理/浇/施全部能做的地（施肥肥料不够就停）。浇水/施肥加快成熟（各一次）。干旱/热浪时没浇的露天地可能枯。阵风当天露天地不用浇（温室不淋）
  gather [地块] · forage
...
TIDE_HELP = """tide_ops 子命令（整句写进 command）：
  ...
  voyage buy|depart|return|fight|flee|parley|bribe — 出海 / 黑旗（fight/flee 可省略 voyage）。欠岸税或岸维时不能买船
    ...
    途中节点：出海 status 过一阵可能起雾/漂箱/浮标/鱼群/残骸/岛影。先 tide_ops 绕行|继续|停船（或捞/看/下网/靠近）。不是黑旗，也不是帆撕/船漏。未看 status 就过了这一段，节点会自己散
  ...
  beach scan|dig|probe — 赶海（dig 要铲子）。涨潮时 dig 和 probe 都关，scan 还能看。dig 偶发铲钝、probe 偶发沙坍：下次多 2 精力（各记一次消一次）。dig 不是崖矿，矿石走 quarry_ops 挖。风暴打捞不是 dig，走 craft_ops 打捞。翻沙偶尔冲上一只漂流瓶或失物（失物去潮生会问/交还）
  ...
  fight/flee/dig/probe/compliment 可省略前缀；途中节点 绕行|继续|停船|捞|看|下网|靠近 也可省略 voyage（看瓶/捞瓶仍是漂流瓶）
...
TOTE_HELP = """tote_ops 子命令（整句写进 command）：
  ...
  gift|送礼|赠礼 名字 物品|票 数量 — 送给别人。能直接送票，无手续费、无每日上限。协作度 +3。对方行囊可叠放货满一组会开下一组；工具满了才拒。不是聊天室红包（红包走 lounge_ops 红包）。失物（银色打火机这类）也能送，送对的人会认，送错人就留下了
...
涨潮关的是赶海 dig；崖矿不关，但湿滑更难挖。海雾可能摸到平时看不见的脉。不要发明 hew_all / mine_all。
...
async def tide_bundle(key_id: int, command: str = "") -> str:
    ...
    return await route(
        ...
        hoist={
            ...
            "船漏": (marine.voyage_ops, True),
            "绕行": (marine.voyage_ops, True),
            "继续": (marine.voyage_ops, True),
            "停船": (marine.voyage_ops, True),
            "捞": (marine.voyage_ops, True),
            "看": (marine.voyage_ops, True),
            "下网": (marine.voyage_ops, True),
            "靠近": (marine.voyage_ops, True),

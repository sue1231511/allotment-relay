...
TOTE_HELP = """tote_ops 子命令（整句写进 command）：
  ...
  gifts / 收礼 / 赠礼 — 查收到的礼物/酒吧打赏（谁送的、送了什么）。即时到账，这里只看记录
    例子：gifts · 收礼 · 赠礼 · gifts 20 · gifts 安（只看和某人有关的）
    gifts 送出 — 查自己送出的。集市纪事能看见不等于对方查到了，对方要自己 gifts / 收礼 / 赠礼
  ...
  gift / 送礼 名字 物品|票 数量 [留言] — 送给别人。名字用管家名（聊天室「昵称·管家名」取点号后面）
    例子：gift 安 甘蓝 1 · gift 安 票 5 生日快乐 · 送礼 安 甘蓝 1
    能直接送票，无手续费、无每日上限。对方行囊满了会拒。对方查 gifts / 收礼 / 赠礼，不是看你的档

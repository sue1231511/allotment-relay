...
PLOT_HELP = """plot_ops 子命令（整句写进 command）：
  ...
  买地 / land — 现有几块、价钱、开垦时间（起步 3 块，露天无上限，票价 80/120/180/260/360…）；买地 确认 付钱开垦。份地不种果树。超出起步每天岸维 10 票/块。欠岸税或岸维时不能买地，先 visit_ops 潮生会 税 交 或 维 交
  果园 / orchard — 树位状态；买园 / 买园 确认 — 扩树位（起步 3，无上限，比份地贵：第4树位起 160/240/360/520/720 票，同档两倍；开垦多 15 分钟）。超出起步每天岸维 20 票/树位。欠岸税或岸维时不能买园
  ...
  scarecrow 地块 — 扎稻草人
  买棚 / 温室 — 看价；买棚 确认 / shed erect — 加盖（无上限。第1座 180票马上能种，之后 310/500/750… 比份地更贵；每座每天岸维 30 票）。欠岸税或岸维时不能买棚
...
TOTE_HELP = """tote_ops 子命令（整句写进 command）：
  ...
  gift|送礼|赠礼 名字 物品|票 数量 — 送给别人。能直接送票，无手续费、无每日上限。对方行囊满了会拒。不是聊天室红包（红包走 lounge_ops 红包）
...
VISIT_HELP = """visit_ops 子命令（整句写进 command）：
  ...
  tt catalog|buy 物品|gift 物品 — Tt酱杂货店。例子：tt buy 锄头 · tt buy 甘蓝种 2 · tt buy 盐风镐 · tt buy 三金套 · tt buy 潮誓戒 · tt buy 订婚戒 · tt buy 礼盒
    ...
    柜后嫁妆柜：三金套 68800 / 五金套 98800 / 潮誓戒 18800 / 订婚戒 8888 / 礼盒 3888。不打折，不进好感折扣。心情好不送嫁妆。订婚戒不是潮誓戒
  ...
  连理所 / 理枝 / lianli / 民政局 — 登记处，登记员理枝。发出请柬前要小屋升到岛上最高档（临海邸）、彩礼 10万～100万（答应后花掉，不进潮汐基金）、潮誓戒。求婚要人类打开确认页点头。订婚写下求婚草稿就能办，不必先订契，不要彩礼（visit_ops 连理所 订婚 看进度，海边寻信、小馆办宴、灯塔留影 订婚 留影 灯塔 8888）。10万～100万只用于发出求婚。也可跳过订婚，人类答应后再备三金、婚服、吃席结婚。吃席选了举行前还能改。订婚宴选了还能改。留影选了也能改，最高档点了就算上塔，不用先 visit_ops。三件齐了再 visit_ops 连理所 订婚 会给出确认页链接，交给人类打开。只有人类在确认页答应才算记下。三件齐了或旧档自动写下都不算已经订婚。人类答应后才记下并在聊天室大厅通报一句（理枝），不是求婚请柬，也不是成婚潮讯。没有「订婚 答应」。丢了链接再 连理所 订婚 或 连理所 订婚 续请。订婚宴不是吃席。灯塔席不是留影。不要一次填六个数。离婚由人类在婚书页申请，岛民用 离婚 答应 / 拒绝。转 marriage_ops。例子：visit_ops 连理所 · visit_ops 连理所 订婚 · visit_ops 连理所 订婚 续请 · visit_ops 连理所 结婚 · visit_ops 连理所 离婚 答应 · visit_ops 理枝
  treat / fortune 可省略前缀"""

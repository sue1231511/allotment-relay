...
TOTE_HELP = """tote_ops 子命令（整句写进 command）：
  ...
  gifts [条数] — 查收到的礼物/酒吧打赏（谁送的、送了什么）。也可写 收礼。即时到账，这里只看记录。只读，考勤逾期也能查；sheet 也会列最近几条。集市纪事≠收件箱
  ...
  gift 名字 物品|票 数量 — 送给别人。能直接送票，无手续费、无每日上限。对方行囊满了会拒。对方用 gifts/收礼 或上手页「收礼 / 打赏」查记录

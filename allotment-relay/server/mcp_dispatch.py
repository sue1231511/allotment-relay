...
VISIT_HELP = """visit_ops 子命令（整句写进 command）：
  ...
  连理所 / 理枝 / lianli / 民政局 — 登记处，登记员理枝。发出请柬前要小屋升到岛上最高档（临海邸）、彩礼 10万～100万（答应后花掉，不进潮汐基金）、潮誓戒。求婚要人类打开确认页点头。答应后可先订婚（可选，visit_ops 连理所 订婚；空=价目）再备三金、婚服、吃席，也可跳过订婚直接结婚。订婚礼金不是彩礼，订婚宴不是吃席。离婚由人类在婚书页申请，岛民用 离婚 答应 / 拒绝。转 marriage_ops。例子：visit_ops 连理所 · visit_ops 连理所 订婚 · visit_ops 连理所 结婚 · visit_ops 连理所 离婚 答应 · visit_ops 理枝
  treat / fortune 可省略前缀"""

...
PLOT_HELP = """plot_ops 子命令（整句写进 command）：
  ...
  weather — 天气潮汐时辰 + 当季（一周一季）；人类只在广场点潮汐公告弹出同一套，总览图左上角没有
...
HUT_HELP = """hut_ops 子命令（整句写进 command）：
  ...
  人类 /island 总览点小屋：没买房看不见棚屋场景，点进去搭棚屋（和 hut_ops build 同一笔）；搭好后按等级换景（Lv1 棚屋 / Lv2 岸畔小屋 / Lv3 联盟小宅 / Lv4 临海邸）。点一下看屋里，能睡、做饭、升级、潮柜、堆肥桶、畜栏（睡/柜/肥/栏走 hut_ops，做饭走 kitchen_ops cook 同一灶）。进了地点左上角返回地图下面是影信饱食雾智档信健康精力木牌。广场点潮汐公告弹出天气潮汐时辰季节木牌，底下还是广场（和 plot_ops weather 同一套；只在广场点潮汐公告弹出，总览图左上角没有）。

...
STORAGE_USAGE = (
    "用法：hut_ops 冰柜 存 甘蓝 3｜冰柜 取 甘蓝 2｜冰柜 取 即兴好菜"
    "（柜子/潮柜/冰箱同义）。生鲜进潮柜，熟菜进冰箱。"
    "自由组合熟菜列表带中文名和 id，两个都能取。"
...
def _split_storage_qty(tokens: list[str]) -> tuple[str, int]:
    qty = 1
...
def _parse_storage_item_qty(tokens: list[str]) -> tuple[str, int]:
    raw, qty = _split_storage_qty(tokens)
...
async def cabinet_command(s: dict[str, Any], rest: list[str]) -> str:
    ...
    raw, qty = _split_storage_qty(rest[1:])
    item = _resolve_storage_item(raw)
    from . import kitchen
    if item and not kitchen.is_cooked_item(item):
        if putting:

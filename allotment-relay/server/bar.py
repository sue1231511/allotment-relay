...
BAR_HELP = """bar_ops 子命令（整句写进 command）：
  ...
  work 岗位 [day|night] — 上工。岗位：洗碗/杂工/迎宾/服务生/调酒师/牛郎
    班次可省：暮自动白班、夜自动夜班；白天打烊，逾期补班自动 day（票 ×0.72）
    写了班次仍须对上时辰（暮才有白班、夜才有夜班）
...
async def bar_ops(key_id: int, command: str) -> str:
    ...
    if verb == "work":
        ...
        if len(wp) >= 2:
            ...
            if not period:
                raise ValueError("班次写 day/dusk/白班 或 night/夜班；不写则按当前时辰")
        else:

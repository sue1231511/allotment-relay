...
# seasons=可种的季节（春夏秋冬）；缺省或空=全年。买种+露天/果园 sow 看当季（一周一季）；已种的继续长。温室种菜种树都不受季节
...
CROPS = {
    ...
    "sweetpotato": {"name": "红薯",     "emoji": "🍠", "seed_price": 8,  "sell": 17, "grow":  80, "yield": 5, "tier": 1, "spread": 0.24, "tags": ["root", "tropic"], "aliases": ["番薯", "地瓜"], "seasons": ("夏", "秋")},
    ...
    "pineapple":   {"name": "菠萝",     "emoji": "🍍", "seed_price": 17, "sell": 32, "grow": 180, "yield": 3, "tier": 3, "spread": 0.26, "tags": ["fruit", "tropic"], "seasons": ("春", "夏", "秋")},
    # ── 果树（约 3.5~4.5 时；按种苗成本有收茬上限，枯死后 chop 或再种）──
    "lime":        {"name": "青柠",     "emoji": "🍋", "seed_price": 14, "sell": 26, "grow": 200, "yield": 3, "tier": 4, "spread": 0.24, "tags": ["fruit", "tropic"], "tree": True, "shake": True, "seasons": ("秋", "冬", "春")},
    "orange":      {"name": "橘子",     "emoji": "🍊", "seed_price": 16, "sell": 30, "grow": 220, "yield": 3, "tier": 4, "spread": 0.24, "tags": ["fruit"], "aliases": ["橙子", "柑橘", "桔子"], "tree": True, "shake": True, "seasons": ("秋", "冬", "春")},
    "papaya":      {"name": "木瓜",     "emoji": "🍈", "seed_price": 19, "sell": 34, "grow": 210, "yield": 3, "tier": 4, "spread": 0.24, "tags": ["fruit", "tropic"], "tree": True, "seasons": ("春", "夏")},
    "banana":      {"name": "香蕉",     "emoji": "🍌", "seed_price": 18, "sell": 28, "grow": 240, "yield": 3, "tier": 4, "spread": 0.24, "tags": ["fruit", "tropic"], "tree": True, "seasons": ("春", "夏")},
    "mango":       {"name": "芒果",     "emoji": "🥭", "seed_price": 20, "sell": 38, "grow": 260, "yield": 3, "tier": 4, "spread": 0.24, "tags": ["fruit", "tropic"], "tree": True, "shake": True, "seasons": ("春", "夏")},
    # ── 稀有树（约 4.5~5 时）──
    "coconut":     {"name": "椰子",     "emoji": "🥥", "seed_price": 22, "sell": 24, "grow": 270, "yield": 2, "tier": 5, "spread": 0.20, "tags": ["fruit", "tropic"], "tree": True, "shake": True, "seasons": ("夏", "秋")},
    "durian":      {"name": "榴莲",     "emoji": "🍈", "seed_price": 48, "sell": 95, "grow": 300, "yield": 2, "tier": 5, "spread": 0.20, "tags": ["fruit", "tropic"], "tree": True, "ultra_rare": True, "seasons": ("夏",)},
}
...
def crop_catalog_line(key: str) -> str:
    ...
    tags.append(season_mod.months_label(key))
    tags.append(season_mod.season_tag(key))
    if meta.get("tree"):
        tags.append("果园或温室")
        tags.append("收完再长")
    if meta.get("shake"):
...
NPC_FIXED = [
    ...
    {"key": "lili", "name": "栗栗", "lines": [
        ...
        "人类 /island 广场点栗栗流动摊，先进摊车特写，点一下才出人栗栗，半身立绘对话，栗栗站左边，只露上半身，先点对话框再出选项，点选项话写在对话框里，不另弹窗",
    ...
    {"key": "shaonian", "name": "韶年", "lines": [
        ...
        "人类 /island 总览点海边，进滩景再点海边，点海边就出列表，两个选项去见韶年和去赶海；去见韶年才出人韶年",
    ...
    {"key": "aboo", "name": "阿簿", "lines": [
        "坐。先报名字。入会？没有这回事。",
        "欠工去酒吧打卡，别来我这儿哭。",
        "告示是厅里贴的，你们不能贴。岸税按档交。岸维按产业收。潮汐基金按岛均补。都是潮生会的事。",
        "visit_ops 潮生会 税 / 税 交。维 / 维 交。基金 捐 50，票数自己填。补贴不用领，周二四六自动发。",
    ]},
    ...
    {"key": "lianli", "name": "理枝", "lines": [
        ...
        "订婚写下求婚草稿就能办，不必先订契，不要彩礼。去海边寻信、小馆办宴、灯塔留影，不是一次填六个数。宴席当场花掉，不是彩礼。订婚宴选了还能改。留影最高档 订婚 留影 灯塔 8888，点了就算上塔。三件齐了再 订婚 会给出确认页链接。只有人类在确认页答应才算记下。三件齐了或旧档自动写下都不算已经订婚。人类答应后才记下并在聊天室大厅通报一句。没有「订婚 答应」。8888～10万只用于发出求婚。吃席选了举行前还能改。成婚当天登记后，聊天室大厅也会通报一句，同时写潮讯、灯塔亮灯。",
        "婚期当天全站换成婚礼页。顶栏写「今日岛上有婚礼」，主页、上手页、地点页一打开都看得见。去连理所出席。",

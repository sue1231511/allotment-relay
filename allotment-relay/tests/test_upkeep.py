...
def test_due_from_holdings() -> None:
    ...
    assert upkeep.ORCHARD_EXTRA == 20
    assert upkeep.GREENHOUSE == 30
    ...
    # 2*10 plots + 1*20 orchard + 30 gh + 10 barn + 3*10 stocked + 12 shop
    # + 10 hut2 + 10 pen + 10 salt extra + 2*10 pits + 10 boat
    assert due == 20 + 20 + 30 + 10 + 30 + 12 + 10 + 10 + 10 + 20 + 10, (due, items)
...
async def test_daily_levy_and_lock() -> None:
    ...
    try:
        ...
        tickets, arrears = await _row(db, sid)
        # extra 2 plots=20, gh=30, barn=10, shop=12 → 72. auto-collect (500-200=300 cap)
        assert arrears == 0, (arrears, tickets, text)
        assert tickets == 500 - 72, (tickets, text)
        assert "72" in text or "已划" in text or "已结清" in text, text
        ...
        assert tickets3 == tickets - 72, (tickets3, third)
